            ...
            struct InternalHostResolutor: dg::network_producer_consumer::KVConsumerInterface<host_ptr_t, HostResolutorArgument>{
                ...
                void push(host_ptr_t, HostResolutorArgument * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        auto work_order             = [dst, src, cpy_dst, tileops_dispatch_control]() noexcept{
                            dg::network_exception_handler::nothrow_log(dg::network_tileops_host_poly::forward_mono(dst, src, tileops_dispatch_control));
                            dg::network_exception_handler::nothrow_log(dg::network_memops_clib::memcpy_host_to_host(cpy_dst, dst, dg::network_tileops_host_poly::decode_mono_dispatch_control(tileops_dispatch_control)->byte_size));
                        };
                        auto virtual_wo             = dg::network_host_asynchronous::virtualize_async_task(std::move(work_order)); //TODOs: optimizables
                        dg::network_exception_handler::nothrow_log(virtual_wo_vec->add(std::move(virtual_wo)));
                    }
                ...
                void push(cuda_ptr_t, CudaResolutorArgument * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        auto work_order         = [e = data_arr[i]]() noexcept{
                            dg::network_exception_handler::nothrow_log(dg::network_tileops_cuda_poly::backward_mono(e.src_grad_addr, e.src_logit_addr, e.dst_grad_addr, e.dispatch_control)); //TODOs: src_grad_status
                        };
                        auto virtual_wo         = dg::network_cuda_controller::virtualize_async_task(std::move(work_order)); //TODOs: optimizables
                        dg::network_exception_handler::nothrow_log(virtual_wo_vec->add(std::move(virtual_wo)));
                    }
            ...
            struct InternalHostResolutor: dg::network_producer_consumer::KVConsumerInterface<host_ptr_t, HostResolutorArgument>{
                ...
                void push(host_ptr_t, HostResolutorArgument * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        auto work_order         = [e = data_arr[i]]() noexcept{
                            dg::network_exception_handler::nothrow_log(dg::network_tileops_host_poly::backward_mono(e.src_grad_addr, e.src_logit_addr, e.dst_grad_addr, e.dispatch_control)); //TODOs: src_grad_status
                        };
                        auto virtual_wo         = dg::network_host_asynchronous::virtualize_async_task(std::move(work_order)); //TODOs: optimizables
                        dg::network_exception_handler::nothrow_log(virtual_wo_vec->add(std::move(virtual_wo)));
                    }
    ...
    class BackwardDoPairSignalResolutorV2: public virtual dg::network_producer_consumer::ConsumerInterface<BackwardDoSignalEvent>{
            ...
            void push(BackwardDoSignalEvent * event_arr, size_t sz) noexcept{
...

                {
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        fetch_arg.root                  = event_arr[i].dst;
                        fetch_arg.expected_ops_id       = event_arr[i].operatable_id;
                        fetch_arg.lhs_rhs_fetching_addr = std::next(descendant_arr.get(), i); 
...

                {
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        auto resolutor_val              = ResolutorValueArgument{};
                        resolutor_val.dst               = event_arr[i].dst;
                        resolutor_val.lhs               = lhs_ptr;
                        resolutor_val.rhs               = rhs_ptr;
                        resolutor_val.expected_ops_id   = event_arr[i].operatable_id;
        ...
        private:
            ...
            struct AddressFetcherArgument{
                uma_ptr_t root;
                operatable_id_t expected_ops_id;
                std::optional<std::tuple<uma_ptr_t, uma_ptr_t>> * lhs_rhs_fetching_addr;
            };
                ...
                void push(uma_ptr_t rcu_addr, AddressFetcherArgument * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        switch (init_status){
                            ...
                            {
                                if (current_ops_id == data_arr[i].expected_ops_id){
                                    *data_arr[i].lhs_rhs_fetching_addr = std::make_tuple(dg::network_tile_member_getsetter::get_pair_left_descendant_nothrow(data_arr[i].root), 
                                                                                         dg::network_tile_member_getsetter::get_pair_right_descendant_nothrow(data_arr[i].root));
                                }
            ...
            struct InternalCudaMixedResolutor: dg::network_producer_consumer::ConsumerInterface<CudaResolutorArgument>{
                ...
                void push(CudaResolutorArgument * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        dg::network_exception_handler::nothrow_log(dg::network_tileops_cuda_poly::backward_pair_dispatch_chk(data_arr[i].lhs_logit_ptr, data_arr[i].lhs_grad_ptr, 
                                                                                                                             data_arr[i].rhs_logit_ptr, data_arr[i].rhs_grad_ptr, 
                                                                                                                             data_arr[i].dst_grad_ptr, data_arr[i].dispatch_control));

                        total_complexity        += dg::network_tileops_cuda_poly::decode_backward_pair_dispatch_control(data_arr[i].dispatch_control)->runtime_complexity;
                        auto work_order         = [e = data_arr[i]]() noexcept{
                            dg::network_exception_handler::nothrow_log(dg::network_tileops_cuda_poly::backward_pair(e.lhs_logit_ptr, e.lhs_grad_ptr, 
                                                                                                                    ...
                                                                                                                    e.dst_grad_ptr, e.dispatch_control)); //TODOs: grad status
                        };
                        auto virtual_wo         = dg::network_cuda_controller::virtualize_async_task(std::move(work_order)); //TODOs: optimizables
                        dg::network_exception_handler::nothrow_log(virtual_wo_vec->add(std::move(virtual_wo))); 
                    }
            ...
            struct InternalHostMixedResolutor: dg::network_producer_consumer::ConsumerInterface<HostResolutorArgument>{
                ...
                void push(HostResolutorArgument * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        dg::network_exception_handler::nothrow_log(dg::network_tileops_host_poly::backward_pair_dispatch_chk(data_arr[i].lhs_logit_ptr, data_arr[i].lhs_grad_ptr, 
                                                                                                                             data_arr[i].rhs_logit_ptr, data_arr[i].rhs_grad_ptr, 
                                                                                                                             data_arr[i].dst_grad_ptr, data_arr[i].dispatch_control));

                        total_complexity        += dg::network_tileops_host_poly::decode_backward_pair_dispatch_control(data_arr[i].dispatch_control)->runtime_complexity;
                        auto work_order         = [e = data_arr[i]]() noexcept{
                            dg::network_exception_handler::nothrow_log(dg::network_tileops_host_poly::backward_pair(data_arr[i].lhs_logit_ptr, data_arr[i].lhs_grad_ptr,
                                                                                                                    data_arr[i].rhs_logit_ptr, data_arr[i].rhs_grad_ptr,
                                                                                                                    data_arr[i].dst_grad_ptr, data_arr[i].dispatch_control)); //TODOs: grad_status
                        };
                        auto virtual_wo         = dg::network_host_asynchronous::virtualize_async_task(std::move(work_order)); //TODOs: optimizables
                        dg::network_exception_handler::nothrow_log(virtual_wo_vec->add(std::move(virtual_wo)));
                    }
            ...
            struct InternalResolutor: dg::network_producer_consumer::KVConsumerInterface<ResolutorKeyArgument, ResolutorValueArgument>{
                ...
                void push(ResolutorKeyArgument key, ResolutorValueArgument * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        if (!dg::network_uma::reacquirer_fixedsize_is_region_reacquirable(umamap_reacquirer, {{lhs_logit_umaptr.value(), dispatch_info.lhs_logit_vd_id},
                        ...
                        if (dg::network_dispatch_control::is_cuda_dispatch(dispatch_info.dispatch_platform)){
                            ...
                            if (dst_dispatch_major == PAIR_DISPATCH_MAJOR_RIGHT){
                                dg::network_producer_consumer::delvrsrv_deliver(cuda_delivery_handle.get(), cuda_resolutor_arg.rhs_logit_ptr, cuda_resolutor_arg);
                            } else if (dst_dispatch_major == PAIR_DISPATCH_MAJOR_LEFT){
                                dg::network_producer_consumer::delvrsrv_deliver(cuda_delivery_handle.get(), cuda_resolutor_arg.lhs_logit_ptr, cuda_resolutor_arg);
                            } else if (dst_dispatch_major == PAIR_DISPATCH_MAJOR_MIXED){
                                dg::network_producer_consumer::delvrsrv_deliver(cuda_mixed_delivery_handle.get(), cuda_resolutor_arg); //TODOs: optimizables
                            } else{
                                if constexpr(DEBUG_MODE_FLAG){
                            ...
                            if (dst_dispatch_major == PAIR_DISPATCH_MAJOR_RIGHT){
                                dg::network_producer_consumer::delvrsrv_deliver(host_delivery_handle.get(), host_resolutor_arg.rhs_logit_ptr, host_resolutor_arg);
                            } else if (dst_dispatch_major == PAIR_DISPATCH_MAJOR_LEFT){
                                dg::network_producer_consumer::delvrsrv_deliver(host_delivery_handle.get(), host_resolutor_arg.lhs_logit_ptr, host_resolutor_arg);
                            } else if (dst_dispatch_major == PAIR_DISPATCH_MAJOR_MIXED){
                                dg::network_producer_consumer::delvrsrv_deliver(host_mixed_delivery_handle.get(), host_resolutor_arg); //TODOs: optimizables
                            } else{
                                if constexpr(DEBUG_MODE_FLAG){
    ...
    class BackwardDoExtnSrcSignalResolutorV2: public virtual dg::network_produer_consumer::ConsumerInterface<BackwardDoSignalEvent>{
            ...
            void push(BackwardDoSignalEvent * event_arr, size_t sz) noexcept{
...

                {
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        fetch_arg.root                      = event_arr[i].dst;
                        fetch_arg.expected_ops_id           = event_arr[i].operatable_id;
                        fetch_arg.src_ctrprt_fetching_addr  = std::next(descendant_localcounterpart_arr.get(), i);
...

                {
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        auto resolutor_key_arg              = ResolutorKeyArgument{};
                        resolutor_key_arg.dst_lck_addr      = dg::memult::region(dst_rcu_addr, lck_region_sz);
                        resolutor_key_arg.src_lck_addr      = dg::memult::region(descendant_rcu_addr.value(), lck_region_sz);
                        resolutor_key_arg.ctrprt_lck_addr   = dg::memult::region(local_counterpart_rcu_addr.value(), lck_region_sz);  
                        auto resolutor_val_arg              = ResolutorValueArgument{};
                        resolutor_val_arg.dst               = event_arr[i].dst;
                        resolutor_val_arg.src               = descendant;
                        resolutor_val_arg.ctrprt            = local_counterpart;
                        resolutor_val_arg.expected_ops_id   = event_arr[i].operatable_id;
        ...
        private:
            ...
            struct AddressFetcherArgument{
                uma_ptr_t root;
                operatable_id_t expected_ops_id;
                std::optional<std::tuple<uma_ptr_t, uma_ptr_t>> * src_ctrprt_fetching_addr;
            };
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        switch (init_status){
                            ...
                            case TILE_INIT_STATUS_INITIALIZED:
                            {
                                ...
                                uma_ptr_t descendant    = dg::network_tile_member_getsetter::get_extnsrc_descendant_nothrow(data_arr[i].root);
                                uma_ptr_t counterpart   = dg::network_tile_member_getsetter::get_extnsrc_counterpart_nothrow(data_arr[i].root);
                                std::expected<std::optional<uma_ptr_t>, exception_t> local_counterpart = this->alias_getter->alias(counterpart);

                                if (local_counterpart.has_value() && local_counterpart.value().has_value()){
                                    *data_arr[i].src_ctrprt_fetching_addr = std::make_tuple(descendant, local_counterpart.value().value());
                                }
            ...
            struct InternalCudaResolutor: dg::network_producer_consumer::KVConsumerInterface<cuda_ptr_t, CudaResolutorArgument>{
                ...
                void push(cuda_ptr_t, CudaResolutorArgument * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        dg::network_exception_handler::nothrow_log(dg::network_tileops_cuda_poly::backward_mono_dispatch_chk(data_arr[i].src_grad_ptr, data_arr[i].src_logit_ptr, 
                                                                                                                             data_arr[i].dst_grad_ptr, data_arr[i].dispatch_control));
                        ...
                        auto work_order         = [e = data_arr[i]]() noexcept{
                            dg::network_exception_handler::nothrow_log(dg::network_tileops_cuda_poly::backward_mono(e.src_grad_ptr, e.src_logit_ptr, e.dst_grad_ptr, e.dispatch_control)); //TODOs: grad_status
                        };
                        auto virtual_wo         = dg::network_cuda_controller::virtualize_async_task(std::move(work_order)); //TODOs: optimizables
                        dg::network_exception_handler::nothrow_log(virtual_wo_vec->add(std::move(virtual_wo))); 
                    }
            ...
            struct InternalHostResolutor: dg::network_producer_consumer::KVConsumerInterface<host_ptr_t, HostResolutorArgument>{
                ...
                void push(host_ptr_t, HostResolutorArgument * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        dg::network_exception_handler::nothrow_log(dg::network_tileops_host_poly::backward_mono_dispatch_chk(data_arr[i].src_grad_ptr, data_arr[i].src_logit_ptr, 
                                                                                                                             data_arr[i].dst_grad_ptr, data_arr[i].dispatch_control));

                        total_complexity        += dg::network_tileops_host_poly::decode_mono_dispatch_control(data_arr[i].dispatch_control)->runtime_complexity;
                        auto work_order         = [e = data_arr[i]]() noexcept{
                            dg::network_exception_handler::nothrow_log(dg::network_tileops_host_poly::backward_mono(e.src_grad_ptr, e.src_logit_ptr, e.dst_grad_ptr, e.dispatch_control)); //TODOs: grad_status
                        };
                        auto virtual_wo         = dg::network_host_asynchronous::virtualize_async_task(std::move(work_order)); //TODOs: optimizables
                        dg::network_exception_handler::nothrow_log(virtual_wo_vec->add(std::move(virtual_wo)));
                    }
            ...
            struct ResolutorKeyArgument{
                uma_ptr_t dst_lck_addr;
                uma_ptr_t src_lck_addr;
                uma_ptr_t ctrprt_lck_addr;

                template <class Reflector>
                constexpr void dg_reflect(const Reflector& reflector) const noexcept{
                    reflector(dst_lck_addr, src_lck_addr, ctrprt_lck_addr);
                }
            ...
            struct InternalResolutor: dg::network_producer_consumer::KVConsumerInterface<ResolutorKeyArgument, ResolutorValueArgument>{
                ...
                void push(ResolutorKeyArgument key, ResolutorValueArgument * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        auto [src, dst, localcounterpart, expected_ops_id] = std::make_tuple(data_arr[i].dst, data_arr[i].src, data_arr[i].ctrprt, data_arr[i].expected_ops_id);
                        ...
                        operatable_id_t localcounterpart_ops_id         = dg::network_tile_member_getsetter::get_extndst_operatable_memevent_id_nothrow(localcounterpart);
                        operatable_id_t localcounterpart_bwd_ops_id     = dg::network_tile_member_getsetter::get_extndst_operatable_backward_id_nothrow(localcounterpart);
                        ...
                        if (localcounterpart_ops_id != expected_ops_id){
                            continue;
                        }

                        if (localcounterpart_bwd_ops_id != dst_bwd_operatable_id){
                            continue;
                        }
                        ...
                        if (dg::network_dispatch_control::is_cuda_dispatch(dispatch_info.dispatch_platform)){
                            ...
                            cuda_resolutor_arg.src_logit_ptr        = dg::network_vmamap::get_cuda_ptr(src_logit_vmamap_reacquirer);
                            cuda_resolutor_arg.dst_grad_ptr         = dg::network_vmamap::get_cuda_ptr(localcounterpart_grad_vmamap_reacquirer);
                            cuda_resolutor_arg.dispatch_control     = dispatch_info.tileops_cuda_dispatch_control;
                            cuda_resolutor_arg.src_grad_status      = src_grad_status.value();
                            ...
                            host_resolutor_arg.src_logit_ptr        = dg::network_vmamap::get_host_ptr(src_logit_vmamap_reacquirer);
                            host_resolutor_arg.dst_grad_ptr         = dg::network_vmamap::get_host_ptr(localcounterpart_grad_vmamap_reacquirer);
                            host_resolutor_arg.dispatch_control     = dispatch_info.tileops_host_dispatch_control;
                            host_resolutor_arg.src_grad_status      = src_grad_status.value();
                        ...
                        if (dg::network_dispatch_control::is_host_dispatch(dispatch_info.dispatch_platform)){
                            ...
                            *request_ptr                    = Request<external_virtual_memory_event_t>{};
                            ...
                            auto executable                 = [dst, dst_logit_hostptr, dst_grad_hostptr, 
                                ...
                                auto inject_event               = dg::network_exception_handler::nothrow_log(dg::network_external_memcommit_factory::make_event_shadow_injection(tile_data.counterpart, TILE_KIND_EXTNDST, dg::network_compact_serializer::serialize<dg::string>(tile_data)));
                                auto signal_event               = dg::network_exception_handler::nothrow_log(dg::network_external_memcommit_factory::make_event_backward_do_signal(tile_data.counterpart, expected_ops_id));
            ...
            struct InternalCudaResolutor: dg::network_producer_consumer::KVConsumerInterface<cuda_ptr_t, CudaResolutorArgument>>{
                ...
                void push(cuda_ptr_t, CudaResolutorArgument * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        dg::network_exception_handler::nothrow_log(dg::network_tileops_cuda_poly::backward_crit_dispatch_chk(data_arr[i].src_grad_ptr, data_arr[i].src_logit_ptr, 
                                                                                                                             data_arr[i].dst_grad_ptr, data_arr[i].dispatch_control));
                        ...
                        auto work_order         = [e = data_arr[i]]() noexcept{
                            dg::network_exception_handler::nothrow_log(dg::network_tileops_cuda_poly::backward_crit(e.src_grad_ptr, e.src_logit_ptr, e.dst_grad_ptr, e.dispatch_control)); //TODOs: grad_status
                        };
                        auto virtual_wo         = dg::network_cuda_controller::virtualize_async_task(std::move(work_order)); //TODOs: optimizables
                        dg::network_exception_handler::nothrow_log(virtual_wo_vec->add(std::move(virtual_wo)));
                    }
            ...
            struct InternalHostResolutor: dg::network_producer_consumer::KVConsumerInterface<host_ptr_t, HostResolutorArgument>{
                ...
                void push(host_ptr_t, HostResolutorArgument * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        dg::network_exception_handler::nothrow_log(dg::network_tileops_host_poly::backward_crit_dispatch_chk(data_arr[i].src_grad_ptr, data_arr[i].src_logit_ptr,
                                                                                                                             data_arr[i].dst_grad_ptr, data_arr[i].dispatch_control));
                        ...
                        auto work_order         = [e = data_arr[i]]() noexcept{
                            dg::network_exception_handler::nothrow_log(dg::network_tileops_host_poly::backward_crit(e.src_grad_ptr, e.src_logit_ptr, e.dst_grad_ptr, e.dispatch_control)); //TODOs: grad_status
                        };
                        auto virtual_wo         = dg::network_host_asynchronous::virtualize_async_task(std::move(work_order)); //TODOs: optimizables
                        dg::network_exception_handler::nothrow_log(virtual_wo_vec->add(std::move(virtual_wo)));
                    }
            ...
            struct InternalResolutor: dg::network_producer_consumer::KVConsumerInterface<ResolutorKeyArgument, ResolutorValueArgument>{
                ...
                void push(ResolutorKeyArgument key, ResolutorValueArgument * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        if (!dg::network_umamap::reacquirer_fixedsize_is_region_reacquirable(umamap_reacquirer, {{src_grad_umaptr.value(), dispatch_info.src_grad_vd_id}, 
                                                                                                                 {src_logit_umaptr.value(), dispatch_info.src_logit_vd_id},
                                                                                                                 {dst_grad_umaptr, dispatch_info.dst_grad_vd_id}})){
        ...
        private:
            ...
            struct InternalResolutor: dg::network_producer_consumer::KVConsumerInterface<ResolutorKeyArgument, ResolutorValueArgument>{
                ...
                void push(ResolutorKeyArgument key, ResolutorValueArgument * data_arr, size_t sz) noexcept{
                    ...
                    size_t cv_allocation_cost                       = dg::network_producer_consumer::delvrsrv_kv_allocation_cost(&cuda_internal_resolutor, trimmed_cuda_vectorizer_sz);
                    dg::network_stack_allocation::NoExceptRawAllocation<char[]> cv_mem(cv_allocation_cost);
                    auto cuda_vectorizer                            = dg::network_exception_handler::nothrow_log(dg::network_producer_consumer::delvrsrv_open_kv_preallocated_raiihandle(&cuda_internal_resolutor, trimmed_cuda_vectorizer_sz, cv_mem.get()));

                    size_t trimmed_host_vectorizer_sz               = std::min(this->vectorization_sz, sz);
                    size_t hv_allocation_cost                       = dg::network_producer_consumer::delvrsrv_kv_allocation_cost(&host_internal_resolutor, trimmed_host_vectorizer_sz);
                    dg::network_stack_allocation::NoExceptRawAllocation<char[]> hv_mem(hv_allocation_cost);
                    auto host_vectorizer                            = dg::network_exception_handler::nothrow_log(dg::network_producer_consumer::delvrsrv_open_kv_preallocated_raiihandle(&host_internal_resolutor, trimmed_host_vectorizer_sz, hv_mem.get()));
