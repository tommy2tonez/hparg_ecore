...
namespace dg::network_memcommit_resolutor{
    ...
    //string_approx is actually this guy responsibility - we'll talk in depth about the deployment of the massive parallel system - and fully synchronized backward + packet rate later
    //the problem with string approx is there is no problem - it does not tilt the plane to fit the x y z, but it needs crazy amount of data (the massful objects) - to pull data around in the multidimensional space (the dents in the space) - we'll leverage this and fast math_approx in order to do so
    //note that we are still doing continuity compression (yet more forgiven in the sense of gradients and moving towards the values) and the latter method is not "worse" than the plane method in the sense of "logits required to compress data" - we'll prove this (this is from the calibrated layers)
    ...
    class ForwardDoBlkrSignalResolutor: public virtual dg::network_producer_consumer::ConsumerInterface<ForwardDoSignalEvent>{
        ...
        public:
            ...
            void push(ForwardDoSignalEvent * event_arr, size_t sz) noexcept{
                ...
                {
                    InternalAddressFetcher fetcher              = {};
        ...
        private:
            ...
            struct InternalAddressFetcher: dg::network_producer_consumer::KVConsumerInterface<uma_ptr_t, std::tuple<uma_ptr_t, operatable_id_t, std::optional<uma_ptr_t> *>>{

                void push(uma_ptr_t lck_addr, std::tuple<uma_ptr_t, operatable_id_t, std::optional<uma_ptr_t> *> data_arr, size_t sz) noexcept{
            ...
            struct InternalCudaResolutor: dg::network_producer_consumer::KVConsumerInterface<cuda_ptr_t, std::tuple<cuda_ptr_t, cuda_ptr_t, cuda_tileops_dispatch_control_t>>{
                ...
                void push(cuda_ptr_t, std::tuple<cuda_ptr_t, cuda_ptr_t, cuda_tileops_dispatch_control_t> * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        auto [dst, src, tileops_dispatch_control]   = data_arr[i];
                        cuda_ptr_vec[i * 2]                         = dst;
                        cuda_ptr_vec[i * 2 + 1]                     = src;
                        dg::network_exception_handler::nothrow_log(dg::network_tileops_cuda_poly::forward_mono_dispatch_chk(dst, src, tileops_dispatch_control));
                        total_complexity                            += dg::network_tileops_cuda_poly::forward_mono_complexity(tileops_dispatch_control);
                        auto work_order                             = [dst, src, tileops_dispatch_control]() noexcept{
                            dg::network_exception_handler::nothrow_log(dg::network_tileops_cuda_poly::forward_mono(dst, src, tileops_dispatch_control)); //kernel launches might be expensive
                        };

                        dg::network_exception_handler::nothrow_log(virtual_wo_vec->add(dg::network_cuda_controller::virtualize_async_task(std::move(work_order))));
                    }
            ...
            struct InternalHostResolutor: dg::network_producer_consumer::KVConsumerInterface<host_ptr_t, std::tuple<host_ptr_t, host_ptr_t, host_tileops_dispatch_control_t>>{
                ...
                void push(host_ptr_t, std::tuple<host_ptr_t, host_ptr_t, host_tileops_dispatch_control_t> * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        auto [dst, src, tileops_dispatch_control]   = data_arr[i];
                        host_ptr_vec[i * 2]                         = dst;
                        host_ptr_vec[i * 2 + 1]                     = src;
                        dg::network_exception_handler::nothrow_log(dg::network_tileops_host_poly::forward_mono_dispatch_chk(dst, src, tileops_dispatch_control));
                        total_complexity                            += dg::network_tileops_host_poly::forward_mono_complexity(tileops_dispatch_control);
                        auto work_order                             = [dst, src, tileops_dispatch_control]() noexcept{
                            dg::network_exception_handler::nothrow_log(dg::network_tileops_host_poly::forward_mono(dst, src, tileops_dispatch_control));
                        };

                        dg::network_exception_handler::nothrow_log(virtual_wo_vec->add(dg::network_host_asynchronous::virtualize_async_task(std::move(work_order))));
                    }
            ...
            struct InternalResolutor: dg::network_producer_consumer::KVConsumerInterface<std::tuple<uma_ptr_t, uma_ptr_t>, std::tuple<uma_ptr_t, uma_ptr_t, operatable_id_t>>{
                ...
                void push(std::tuple<uma_ptr_t, uma_ptr_t> lck_addr, std::tuple<uma_ptr_t, uma_ptr_t, operatable_id_t> * data_arr, size_t sz) noexcept{
                    ...
                    auto dst_vmamap_reacquirer              = dg::network_exception_handler::nothrow_log(dg::network_vmamap::reacquirer_raii_initialize());
                    auto src_vmamap_reacquirer              = dg::network_exception_handler::nothrow_log(dg::network_vmamap::reacquirer_raii_initialize());
                    ...
                    auto cuda_delivery_handle               = dg::network_exception_handler::nothrow_log(dg::network_producer_consumer::delvrsrv_open_kv_preallocated_raiihandle(&cuda_resolutor, trimmed_cuda_vectorization_sz, cdh_mem.get()));
                    ...
                    auto host_delivery_handle               = dg::network_exception_handler::nothrow_log(dg::network_producer_consumer::delvrsrv_open_kv_preallocated_raiihandle(&host_resolutor, trimmed_host_vectorization_sz, hdh_mem.get()));
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        if (!dg::network_uma::reacquirer_fixedsize_is_region_reacquirable(umamap_reacquirer, {{dst_logit_umaptr, dispatch_info.dst_vd_id}, 
                        ...
                        dg::network_uma::reacquirer_fixedsize_reacquire_nothrow(umamap_reacquirer, {{dst_logit_umaptr, dispatch_info.dst_vd_id}, 
                                                                                                    {src_logit_umaptr.value(), dispatch_info.src_vd_id}});

                        vma_ptr_t dst_map_vmaptr    = dg::network_uma::get_vma_ptr(umamap_reacquirer, std::integral_constant<size_t, 0u>{});
                        ...
                        if (dg::network_dispatch_control::is_cuda_dispatch(dispatch_info.dispatch_platform)){
                            cuda_ptr_t dst_logit_cudaptr    = dg::network_vmamap::get_cuda_ptr(dst_vmamap_reacquirer);
                            cuda_ptr_t src_logit_cudaptr    = dg::network_vmamap::get_cuda_ptr(src_vmamap_reacquirer);

                            dg::network_producer_consumer::delvrsrv_deliver(cuda_delivery_handle.get(), src_logit_cudaptr, std::make_tuple(dst_logit_cudaptr, src_logit_cudaptr, dispatch_info.tileops_dispatch_control));
                        } else if (dg::network_dispatch_control::is_host_dispatch(dispatch_info.dispatch_platform)){
                            host_ptr_t dst_logit_hostptr    = dg::network_vmamap::get_host_ptr(dst_vmamap_reacquirer);
                            host_ptr_t src_logit_hostptr    = dg::network_vmamap::get_host_ptr(src_vmamap_reacquirer);

                            dg::network_producer_consumer::delvrsrv_deliver(host_delivery_handle.get(), src_logit_hostptr, std::make_tuple(dst_logit_hostptr, src_logit_hostptr, dispatch_info.tileops_dispatch_control)); 
                        } else{
                            if constexpr(DEBUG_MODE_FLAG){
                                dg::network_log_stackdump::critical(dg::network_exception::verbose(dg::network_exception::INTERNAL_CORRUPTION));
                                std::abort();
        ...
        private:
            ...
            struct InternalDescendantAddressFetcher: dg::network_producer_consumer::KVConsumerInterface<uma_ptr_t, std::tuple<uma_ptr_t, operatable_id_t, std::optional<uma_ptr_t> *>>{
                ...
                void push(uma_ptr_t rcu_addr, std::tuple<uma_ptr_t, operatable_id_t, std::optional<uma_ptr_t> *> * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        auto [ptr, expected_ops_id, fetching_ptr]   = data_arr[i];
                        init_status_t init_status                   = dg::network_tile_member_getsetter::get_mono_init_status_nothrow(ptr);
                        operatable_id_t current_ops_id              = dg::network_tile_member_getsetter::get_mono_operatable_memevent_id_nothrow(ptr);
            ...
            struct InternalCudaResolutor: dg::network_producer_consumer::KVConsumerInterface<cuda_ptr_t, std::tuple<cuda_ptr_t, cuda_tileops_dispatch_control_t>>{
                ...
                void push(cuda_ptr_t key, std::tuple<cuda_ptr_t, cuda_ptr_t, cuda_tileops_dispatch_control_t> * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        auto [dst_logit_cudaptr, src_logit_cudaptr, tileops_dispatch_control] = data_arr[i];
                        cuda_ptr_vec[i * 2]         = dst_logit_cudaptr;
                        cuda_ptr_vec[i * 2  + 1]    = src_logit_cudaptr;
                        dg::network_exception_handler::nothrow_log(dg::network_tileops_cuda_poly::forward_mono_dispatch_chk(dst_logit_cudaptr, src_logit_cudaptr, tileops_dispatch_control));
                        total_complexity            += dg::network_tileops_cuda_poly::decode_mono_dispatch_control(tileops_dispatch_control)->runtime_complexity;
                        auto work_order             = [dst_logit_cudaptr, src_logit_cudaptr, tileops_dispatch_control]() noexcept{
                            dg::network_exception_handler::nothrow_log(dg::network_tileops_cuda_poly::forward_mono(dst_logit_cudaptr, src_logit_cudaptr, tileops_dispatch_control));
                        };
                    ...
                    dg::network_exception_handler::nothrow_log(this->restrict_synchronizer->add(cuda_ptr_vec.get(), std::next(cuda_ptr_vec.get(), cuda_ptr_vec)));
                    auto synchronizable = dg::network_exception_handler::nothrow_log(this->async_device->exec(std::move(virtual_wo_vec), total_complexity)); //this requires an error issue - we'll work on this later - next sprint
                    dg::network_exception_handler::nothrow_log(this->synchronizer->add(std::move(synchronizable)));
                }
            };
            ...
            struct InternalHostResolutor: dg::network_producer_consumer::KVConsumerInterface<host_ptr_t, std::tuple<host_ptr_t, host_tileops_dispatch_control_t>>{
                ...
                void push(host_ptr_t key, std::tuple<host_ptr_t, host_ptr_t, host_tileops_dispatch_control_t> * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        auto [dst_logit_hostptr, src_logit_hostptr, tileops_dispatch_control] = data_arr[i];
                        host_ptr_vec[i * 2]     = dst_logit_hostptr;
                        host_ptr_vec[i * 2 + 1] = src_logit_hostptr;
                        dg::network_exception_handler::nothrow_log(dg::network_tileops_host_poly::forward_mono_dispatch_chk(dst_logit_hostptr, src_logit_hostptr, tileops_dispatch_control));
                        total_complexity        += dg::network_tileops_host_poly::decode_mono_dispatch_control(tileops_dispatch_control)->runtime_complexity;
                        auto work_order         = [dst_logit_hostptr, src_logit_hostptr, tileops_dispatch_control]() noexcept{
                            dg::network_exception_handler::nothrow_log(dg::network_tileops_host_poly::forward_mono(dst_logit_hostptr, src_logit_hostptr, tileops_dispatch_control));
                        };
            ...
            struct InternalResolutor: dg::network_producer_consumer::KVConsumerInterface<std::tuple<uma_ptr_t, uma_ptr_t>, std::tuple<uma_ptr_t, uma_ptr_t, operatable_id_t>>{
                ...
                void push(std::tuple<uma_ptr_t, uma_ptr_t> lck_addr, std::tuple<uma_ptr_t, uma_ptr_t> * data_arr, size_t sz) noexcept{
                    ...
                    size_t cdh_allocation_cost                      = dg::network_producer_consumer::delvrsrv_kv_allocation_cost(&internal_cuda_resolutor, cdh_trimmed_vectorization_sz);
                    dg::network_stack_allocation::NoExceptRawAllocation<char[]> cdh_mem(cdh_allocation_cost);  
                    ...
                    size_t hdh_allocation_cost                      = dg::network_producer_consumer::delvrsrv_kv_allocation_cost(&internal_host_resolutor, hdh_trimmed_vectorization_sz);
                    dg::network_stack_allocation::NoExceptRawAllocation<char[]> hdh_mem(hdh_allocation_cost);
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        if (dg::network_dispatch_control::is_cuda_dispatch(dispatch_info.dispatch_platform)){
                            ...
                            dg::network_producer_consumer::delvrsrv_deliver(cuda_delivery_handle.get(), src_logit_cudaptr, std::make_tuple(dst_logit_cudaptr, src_logit_cudaptr, dispatch_info.tileops_cuda_dispatch_control));
                        } else if (dg::network_dispatch_control::is_host_dispatch(dispatch_info.dispatch_platform)){
                            ...
                            dg::network_producer_consumer::delvrsrv_deliver(host_delivery_handle.get(), src_logit_hostptr, std::make_tuple(dst_logit_hostptr, src_logit_hostptr, dispatch_info.tileops_host_dispatch_control));
                        } else{
                            if constexpr(DEBUG_MODE_FLAG){
                                dg::network_log_stackdump::critical(dg::network_exception::verbose(dg::network_exception::INTERNAL_CORRUPTION));
                                std::abort();
    ...
    class ForwardDoPairSignalResolutorV2: public virtual dg::network_producer_consumer::ConsumerInterface<ForwardDoSignalEvent>{
        ...
        private:
            ...
            struct InternalDescendantAddressFetcher: dg::network_producer_consumer::KVConsumerInterface<uma_ptr_t, std::tuple<uma_ptr_t, operatable_id_t, std::optional<std::tuple<uma_ptr_t, uma_ptr_t>> *>>{
            ...
            struct InternalCudaMixedResolutor: dg::network_producer_consumer::ConsumerInterface<std::tuple<cuda_ptr_t, cuda_ptr_t, cuda_ptr_t, cuda_tileops_dispatch_control_t>>{

                dg::network_cuda_controller::Synchronizer * synchronizer;
                dg::network_cuda_controller::RestrictPointerSynchronizer * restrict_synchronizer;
                dg::network_cuda_controller::AsynchronousDeviceInterface * async_device;

                void push(std::tuple<cuda_ptr_t, cuda_ptr_t, cuda_ptr_t, cuda_tileops_dispatch_control_t> * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        auto [dst, lhs, rhs, tileops_dispatch_control]  = data_arr[i];
                        cuda_ptr_vec[i * 3]     = dst;
                        cuda_ptr_vec[i * 3 + 1] = lhs;
                        cuda_ptr_vec[i * 3 + 2] = rhs;

                        dg::network_exception_handler::nothrow_log(dg::network_tileops_cuda_poly::forward_pair_dispatch_chk(dst, lhs, rhs, tileops_dispatch_control));
                        total_complexity        += dg::network_tileops_cuda_poly::forward_pair_complexity(tileops_dispatch_control);
                        auto work_order         = [dst, lhs, rhs, tileops_dispatch_control]() noexcept{
                            dg::network_exception_handler::nothrow_log(dg::network_tileops_cuda_poly::forward_pair(dst, lhs, rhs, tileops_dispatch_control));
                        };
                        auto virtual_wo         = dg::network_cuda_controller::virtualize_async_task(std::move(work_order));
                        dg::network_exception_handler::nothrow_log(virtual_wo_vec->add(std::move(virtual_wo)));
                    }
            ...
            struct InternalCudaResolutor: dg::network_producer_consumer::KVConsumerInterface<cuda_ptr_t, std::tuple<cuda_ptr_t, cuda_ptr_t, cuda_ptr_t, cuda_tileops_dispatch_control_t>>{

                dg::network_cuda_controller::AsynchronousDeviceInterface * async_device;
                dg::network_cuda_controller::Synchronizer * synchronizer;
                dg::network_cuda_controller::RestrictPointerSynchronizer * restrict_synchronizer;

                void push(cuda_ptr_t, std::tuple<cuda_ptr_t, cuda_ptr_t, cuda_ptr_t, cuda_tileops_dispatch_control_t> * data_arr, size_t sz) noexcept{

                    auto base_resolutor                     = InternalCudaMixedResolutor{};
                    base_resolutor.async_device             = this->async_device;
                    base_resolutor.synchronizer             = this->synchronizer;
                    base_resolutor.restrict_synchronizer    = this->restrict_synchronizer;
                    base_resolutor.push(data_arr, sz);
                }
            };

            struct InternalHostMixedResolutor: dg::network_producer_consumer::ConsumerInterface<std::tuple<host_ptr_t, host_ptr_t, host_ptr_t, host_tileops_dispatch_control_t>>{

                dg::network_host_asynchronous::AsynchronousDeviceInterface * async_device;
                dg::network_host_asynchronous::Synchronizer * synchronizer;
                dg::network_host_asynchronous::RestrictPointerSynchronizer * restrict_synchronizer;

                void push(std::tuple<host_ptr_t, host_ptr_t, host_ptr_t, host_tileops_dispatch_control_t> * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        dg::network_exception_handler::nothrow_log(dg::network_tileops_host_poly::forward_pair_dispatch_chk(dst, lhs, rhs, tileops_dispatch_control));
                        total_complexity        += dg::network_tileops_host_poly::forward_pair_complexity(tileops_dispatch_control);
                        auto work_order         = [dst, lhs, rhs, tileops_dispatch_control]() noexcept{
                            dg::network_exception_handler::nothrow_log(dg::network_tileops_host_poly::forward_pair(dst, lhs, rhs, tileops_dispatch_control));
                        };
                        auto virtual_wo         = dg::network_host_asynchronous::virtualize_async_task(std::move(work_order));
                        dg::network_exception_handler::nothrow_log(virtual_wo_vec->add(std::move(virtual_wo)));
                    }
            ...
            struct InternalHostResolutor: dg::network_producer_consumer::KVConsumerInterface<host_ptr_t, std::tuple<host_ptr_t, host_ptr_t, host_ptr_t, host_tileops_dispatch_control_t>>{
                ...
                void push(host_ptr_t key, std::tuple<host_ptr_t, host_ptr_t, host_ptr_t, host_tileops_dispatch_control_t> * data_arr, size_t sz) noexcept{
                    ...
                    auto base_resolutor                     = InternalHostMixedResolutor{};
                    base_resolutor.async_device             = this->async_device;
                    base_resolutor.synchronizer             = this->synchronizer;
                    base_resolutor.restrict_synchronizer    = this->restrict_synchronizer;
                    base_resolutor.push(data_arr, sz);
            ...
            struct InternalResolutor: dg::network_producer_consumer::KVConsumerInterface<std::tuple<uma_ptr_t, uma_ptr_t, uma_ptr_t>, std::tuple<uma_ptr_t, uma_ptr_t, uma_ptr_t>>{
                ...
                void push(std::tuple<uma_ptr_t, uma_ptr_t, uma_ptr_t> lck_addr, std::tuple<uma_ptr_t, uma_ptr_t, uma_ptr_t> * data_arr, size_t sz) noexcept{
                    ...
                    auto umamap_reacquirer                              = dg::network_exception_handler::nothrow_log(dg::network_uma::reacquirer_fixedsize_raii_initialize(std::integral_constant<size_t, 3u>{}));
                    auto dst_vmamap_reacquirer                          = dg::network_exception_handler::nothrow_log(dg::network_vmamap::reacquirer_raii_initialize());
                    auto lhs_vmamap_reacquirer                          = dg::network_exception_handler::nothrow_log(dg::network_vmamap::reacquirer_raii_initialize());
                    auto rhs_vmamap_reacquirer                          = dg::network_exception_handler::nothrow_log(dg::network_vmamap::reacquirer_raii_initialize());
                    ...
                    internal_cuda_resolutor.synchronizer                = &cuda_synchronizer;
                    internal_cuda_resolutor.restrict_synchronizer       = &cuda_restrict_synchronizer;
                    auto internal_cuda_mixed_resolutor                  = InternalCudaMixedResolutor{};
                    internal_cuda_mixed_resolutor.async_device          = this->cuda_async_device;
                    internal_cuda_mixed_resolutor.synchronizer          = &cuda_synchronizer;
                    internal_cuda_mixed_resolutor.restrict_synchronizer = &cuda_restrict_synchronizer;
                    ...
                    internal_host_resolutor.synchronizer                = &host_synchronizer;
                    internal_host_resolutor.restrict_synchronizer       = &host_restrict_synchronizer;
                    internal_host_mixed_resolutor.async_device          = this->host_async_device;
                    auto internal_host_mixed_resolutor                  = InternalHostMixedResolutor{};
                    internal_host_mixed_resolutor.synchronizer          = &host_synchronizer;
                    internal_host_mixed_resolutor.restrict_synchronizer = &host_restrict_synchronizer;
                    ...
                    size_t cmdh_allocation_cost                         = dg::network_producer_consumer::delvrsrv_allocation_cost(&internal_cuda_mixed_resolutor, trimmed_cuda_mixed_vectorization_sz);
                    dg::network_stack_allocation::NoExceptRawAllocation<char[]> cmdh_mem(cmdh_allocation_cost);
                    auto cuda_mixed_delivery_handle                     = dg::network_exception_handler::nothrow_log(dg::network_producer_consumer::delvrsrv_open_preallocated_raiihandle(&internal_cuda_mixed_resolutor, trimmed_cuda_mixed_vectorization_sz, cmdh_mem.get())); 
                    ...
                    size_t hmdh_allocation_cost                         = dg::network_producer_consumer::delvrsrv_allocation_cost(&internal_host_mixed_resolutor, trimmed_host_mixed_vectorization_sz);
                    dg::network_stack_allocation::NoExceptRawAllocation<char[]> hmdh_mem(hmdh_allocation_cost);
                    auto host_mixed_delivery_handle                     = dg::network_exception_handler::nothrow_log(dg::network_producer_consumer::delvrsrv_open_preallocated_raiihandle(&internal_host_mixed_resolutor, trimmed_host_mixed_vectorization_sz, hmdh_mem.get()));
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        if (!dg::network_uma::reacquirer_fixedsize_is_region_reacquirable(umamap_reacquirer, {{dst_logit_umaptr, dispatch_info.dst_vd_id}, 
                            ...
                            dg::network_producer_consumer::delvrsrv_clear(cuda_delivery_handle.get());
                            dg::network_producer_consumer::delvrsrv_clear(cuda_mixed_delivery_handle.get());
                            dg::network_producer_consumer::delvrsrv_clear(host_delivery_handle.get());
                            dg::network_producer_consumer::delvrsrv_clear(host_mixed_delivery_handle.get());
                            cuda_synchronizer.sync();
                            host_synchronizer.sync();
                            cuda_restrict_synchronizer.clear();
                            host_restrict_synchronizer.clear();
                        ...
                        if (dg::network_dispatch_control::is_cuda_dispatch(dispatch_info.dispatch_platform)){
                            ...
                            if (dst_dispatch_major == PAIR_DISPATCH_MAJOR_RIGHT){
                                dg::network_producer_consumer::delvrsrv_deliver(cuda_delivery_handle.get(), rhs_cudaptr, std::make_tuple(dst_cudaptr, lhs_cudaptr, rhs_cudaptr, dispatch_info.tileops_cuda_dispatch_control));
                            } else if (dst_dispatch_major == PAIR_DISPATCH_MAJOR_LEFT){
                                dg::network_producer_consumer::delvrsrv_deliver(cuda_delivery_handle.get(), lhs_cudaptr, std::make_tuple(dst_cudaptr, lhs_cudaptr, rhs_cudaptr, dispatch_info.tileops_cuda_dispatch_control));
                            } else if (dst_dispatch_major = PAIR_DISPATCH_MAJOR_MIXED){
                                dg::network_producer_consumer::delvrsrv_deliver(cuda_mixed_delivery_handle.get(), std::make_tuple(dst_cudaptr, lhs_cudaptr, rhs_cudaptr, dispatch_info.tileops_cuda_dispatch_control)); //optimizable
                            } else{
                                if constexpr(DEBUG_MODE_FLAG){
                                    dg::network_log_stackdump::critical(dg::network_exception::verbose(dg::network_exception::INTERNAL_CORRUPTION));
                                    std::abort();
                            ...
                            if (dst_dispatch_major == PAIR_DISPATCH_MAJOR_RIGHT){
                                dg::network_producer_consumer::delvrsrv_deliver(host_delivery_handle.get(), rhs_hostptr, std::make_tuple(dst_hostptr, lhs_hostptr, rhs_hostptr, dispatch_info.tileops_host_dispatch_control));
                            } else if (dst_dispatch_major == PAIR_DISPATCH_MAJOR_LEFT){
                                dg::network_producer_consumer::delvrsrv_deliver(host_delivery_handle.get(), lhs_hostptr, std::make_tuple(dst_hostptr, lhs_hostptr, rhs_hostptr, dispatch_info.tileops_host_dispatch_control));
                            } else if (dst_dispatch_major == PAIR_DISPATCH_MAJOR_MIXED){
                                dg::network_producer_consumer::delvrsrv_deliver(host_mixed_delivery_handle.get(), std::make_tuple(dst_hostptr, lhs_hostptr, rhs_hostptr, dispatch_info.tileops_host_dispatch_control));
                            } else{
                                if constexpr(DEBUG_MODE_FLAG){
                                    dg::network_log_stackdump::critical(dg::network_exception::verbose(dg::network_exception::INTERNAL_CORRUPTION));
                                    std::abort();
            ...
            void push(ForwardDoSignalEvent * event_arr, size_t sz) noexcept{
...

                {
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        uma_ptr_t rcu_addr          = dg::network_tile_member_getsetter::get_uacm_rcu_addr_nothrow(event_arr[i].dst);
                        uma_ptr_t lck_addr          = dg::memult::region(rcu_addr, dg::network_memops_uma::memlock_region_size());
                        uma_ptr_t * addrfetch_addr  = std::next(descendant_arr.get(), i * UACM_ACM_SZ);
                        bool * validation_addr      = std::next(validation_arr.get(), i);  

                        dg::network_producer_consumer::delvrsrv_deliver(vectorized_delivery_handle.get(), lck_addr, std::make_tuple(event_arr[i].dst, event_arr[i].operatable_id, addrfetch_addr, validation_addr));
                    }
                }
            ...
            void push(ForwardDoSignalEvent * event_arr, size_t sz) noexcept{
...

                {
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        uma_ptr_t rcu_addr                      = dg::network_tile_member_getsetter::get_pacm_rcu_addr_nothrow(event_arr[i].dst);
                        uma_ptr_t lck_addr                      = dg::memult::region(rcu_addr, dg::network_memops_uma::memlock_region_size());
                        auto addrfetch_arg                      = AddressFetchArgument{}; 
                        addrfetch_arg.root                      = event_arr[i].dst;
                        addrfetch_arg.expected_ops_id           = event_arr[i].operatable_id;
                        addrfetch_arg.fetching_lhs_addr         = std::next(left_descendant_arr.get(), i * PACM_ACM_SZ);
                        addrfetch_arg.fetching_rhs_addr         = std::next(right_descendant_arr.get(), i * PACM_ACM_SZ); 
                        addrfetch_arg.fetching_validation_flag  = std::next(descendant_validation_arr.get(), i);

                        dg::network_producer_consumer::delvrsrv_deliver(vectorized_delivery_handle.get(), lck_addr, addrfetch_arg);
                    }
                }
        ...
        private:
            ...
            struct InternalHostResolutor: dg::network_producer_consumer::KVConsumerInterface<host_ptr_t, std::tuple<host_ptr_t, host_ptr_t, host_ptr_t, host_tileops_dispatch_control_t>>{
                ...
                void push(host_ptr_t key, std::tuple<host_ptr_t, host_ptr_t, host_ptr_t, host_tileops_dispatch_control_t> * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        dg::network_exception_handler::nothrow_log(dg::network_tileops_host_poly::forward_mono_dispatch_chk(dst, src, tileops_dispatch_control));
                        total_complexity        += dg::network_tileops_host_poly::forward_mono_complexity(tileops_dispatch_control);
                        total_complexity        += dg::network_tileops_host_poly::forward_mono_complexity(MEMCPY);
                        auto work_order         = [dst, src, cpy_dst, tileops_dispatch_control]() noexcept{
                            dg::network_exception_handler::nothrow_log(dg::network_tileops_host_poly::forward_mono(dst, src, tileops_dispatch_control));
                            dg::network_exception_handler::nothrow_log(dg::network_memops_clib::memcpy_host_to_host(cpy_dst, src, dg::network_tileops_host_poly::get_mono_byte_size(tileops_dispatch_control)));
                        };
                        auto virtual_wo         = dg::network_host_asynchronous::virtualize_async_task(std::move(work_order));
                        dg::network_exception_handler::nothrow_log(virtual_wo_vec->add(std::move(virtual_wo)));
                    }
                    ...
                    dg::network_exception_handler::nothrow_log(this->restrict_synchronizer->add(host_ptr_vec.get(), std::next(host_ptr_vec.get(), host_ptr_vec_sz)));
                    auto synchronizable = dg::network_exception_handler::nothrow_log(this->host_async_device->exec(std::move(virtual_wo_vec), total_complexity)); //we'll fix this later 
                    dg::network_exception_handler::nothrow_log(this->synchronizer->add(std::move(synchronizable)));
                }
            };
            ...
            struct InternalResolutor: dg::network_producer_consumer::KVConsumerInterface<std::tuple<uma_ptr_t, uma_ptr_t>, std::tuple<uma_ptr_t, uma_ptr_t, operatable_id_t>>{
                ...
                void push(std::tuple<uma_ptr_t, uma_ptr_t> lck_addr, std::tuple<uma_ptr_t, uma_ptr_t> * data_arr, size_t sz) noexcept{
                    ...
                    auto src_logit_vmamap_reacquirer                = dg::network_exception_handler::nothrow_log(dg::network_vmamap::reacquirer_raii_initialize());
                    ...
                    host_internal_resolutor.host_async_device       = this->host_async_device;
                    host_internal_resolutor.synchronizer            = &synchronizer;
                    host_internal_resolutor.restrict_synchronizer   = &restrict_synchronizer;

                    size_t trimmed_vectorization_sz                 = std::min(this->vectorization_sz, sz);
                    size_t hvdh_allocation_cost                     = dg::network_producer_consumer::delvrsrv_kv_allocation_cost(&host_internal_resolutor, trimmed_vectorization_sz);
                    dg::network_stack_allocation::NoExceptRawAllocation<char[]> hvdh_mem(hvdh_allocation_cost);
                    auto host_vectorizer_delivery_handle            = dg::network_exception_handler::nothrow_log(dg::network_producer_consumer::delvrsrv_open_kv_raiihandle(&host_internal_resolutor, trimmed_vectorization_sz, hvdh_mem.get())); 
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        if(dg::network_dispatch_control::is_host_dispatch(dispatch_info.dispatch_platform)){
                            ...
                            rq.extnsrc_metadata_tile        = dg::network_tile_member_getsetter::get_extnsrc_metadata_nothrow(dst);
                            rq.addr                         = dst;
                            rq.logit_buf                    = dg::string(dg::network_tileops_host_poly::get_buffer_size(dispatch_info.tileops_host_dispatch_control), ' ');
                            host_ptr_t rq_buf_data          = rq.logit_buf.data();
                            outbound_data_vec[i]            = std::move(rq);

                            dg::network_producer_consumer::delvrsrv_deliver(host_vectorizer_delivery_handle.get(), src_logit_hostptr, std::make_tuple(dst_logit_hostptr, src_logit_hostptr, rq_buf_data, dispatch_info.tileops_host_dispatch_control));
                        } else{
                            if constexpr(DEBUG_MODE_FLAG){
                                dg::network_log_stackdump::critical(dg::network_exception::verbose(dg::network_exception::INTERNAL_CORRUPTION));
                                std::abort();
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        dg::string serialized                           = dg::network_compact_serializer::serialize<dg::string>(std::move(extnsrc_tile)); 
                        external_virtual_memory_event_t inject_event    = dg::network_external_memcommit_factory::make_event_shadow_injection(outbound_data_vec[i]->extnsrc_metadata_tile.counterpart, TILE_KIND_EXTNSRC, std::move(serialized));
        ...
        private:
            ...
            struct InternalCudaResolutor: dg::network_producer_consumer::KVConsumerInterface<cuda_ptr_t, std::tuple<cuda_ptr_t, cuda_ptr_t, cuda_tileops_dispatch_control_t>>{
                ...
                void push(cuda_ptr_t, std::tuple<cuda_ptr_t, cuda_ptr_t, cuda_tileops_dispatch_control_t> * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        auto [dst, src, tileops_dispatch_control] = data_arr[i];
                        cuda_ptr_arr[i * 2]         = dst;
                        cuda_ptr_arr[i * 2 + 1]     = src;

                        dg::network_exception_handler::nothrow_log(dg::network_tileops_cuda_poly::forward_mono_dispatch_chk(dst, src, tileops_dispatch_control));
                        total_complexity            += dg::network_tileops_cuda_poly::forward_mono_complexity(tileops_dispatch_control);
                        auto work_order             = [dst, src, tileops_dispatch_control]() noexcept{
                            dg::network_exception_handler::nothrow_log(dg::network_tileops_cuda_poly::forward_mono(dst, src, tileops_dispatch_control));
                        };
                        auto virtual_wo             = dg::network_cuda_controller::virtualize_async_task(std::move(work_order));
                        dg::network_exception_handler::nothrow_log(virtual_wo_vec->add(std::move(virtual_wo)));
                    }
            ...
            struct InternalHostResolutor: dg::network_producer_consumer::KVConsumerInterface<host_ptr_t, std::tuple<host_ptr_t, host_ptr_t, host_tileops_dispatch_control_t>>{
                ...
                void push(host_ptr_t, std::tuple<host_ptr_t, host_ptr_t, host_tileops_dispatch_control_t> * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        auto [dst, src, tileops_dispatch_control] = data_arr[i];
                        host_ptr_arr[i * 2]         = dst;
                        host_ptr_arr[i * 2 + 1]     = src;
            ...
            struct InternalResolutor: dg::network_producer_consumer::ConsumerInterface<std::tuple<uma_ptr_t, uma_ptr_t>, std::tuple<uma_ptr_t, uma_ptr_t, operatable_id_t>>{
                ...
                void push(std::tuple<uma_ptr_t, uma_ptr_t> lck_addr, std::tuple<uma_ptr_t, uma_ptr_t> * ptr_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        operatable_id_t src_fwd_operatable_id   = dg::network_tile_member_getsetter::get_extnsrc_operatable_forward_id_nothrow(src);
                        ...
                        if (dg::network_dispatch_control::is_cuda_dispatch(dispatch_info.dispatch_platform)){
                            cuda_ptr_t dst_logit_cudaptr = dg::network_vmamap::get_cuda_ptr(dst_logit_vmamap_reacquirer);
                            cuda_ptr_t src_logit_cudaptr = dg::network_vmamap::get_cuda_ptr(src_logit_vmamap_reacquirer);

                            dg::network_producer_consumer::delvrsrv_deliver(cuda_delivery_handle.get(), src_logit_cudaptr, std::make_tuple(dst_logit_cudaptr, src_logit_cudaptr, dispatch_info.tileops_cuda_dispatch_control)); //we have to vectorize to avoid synchronization overheads - not necessarily because this is a feature
                        } else if (dg::network_dispatch_control::is_host_dispatch(dispatch_info.dispatch_platform)){
                            host_ptr_t dst_logit_hostptr = dg::network_vmamap::get_host_ptr(dst_logit_vmamap_reacquirer);
                            host_ptr_t src_logit_hostptr = dg::network_vmamap::get_host_ptr(src_logit_vmamap_reacquirer);
    ...
    class ForwardDoCritSignalResolutor: public virtual dg::network_producer_consumer::ConsumerInterface<ForwardDoSignalEvent>{
            ...
            void push(ForwardDoSignalEvent * event_arr, size_t sz) noexcept{
...

                {
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        uma_ptr_t src_lck_addr  = dg::memult::region(src_rcu_addr.value(), lck_region_sz);
                        uma_ptr_t dst_lck_addr  = dg::memult::region(dst_rcu_addr.value(), lck_region_sz);
                        auto key                = std::make_tuple(dst_lck_addr, src_lck_addr);
        ...
        private:
            ...
            struct InternalDescendantAddressFetcher: dg::network_producer_consumer::KVConsumerInterface<uma_ptr_t, std::tuple<uma_ptr_t, operatable_id_t, std::optional<uma_ptr_t> *>>{
                ...
                void push(uma_ptr_t rcu_addr, std::tuple<uma_ptr_t, operatable_id_t, std::optional<uma_ptr_t> *> data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        operatable_id_t current_ops_id              = dg::network_tile_member_getsetter::get_crit_operatable_memevent_id_nothrow(dst);
                        ...
                        switch (init_status){
                            ...
                            case TILE_INIT_STATUS_DECAYED:
                            {
                                if (current_ops_id != expected_ops_id){
                                    ...
                                    *fetching_addr = dg::network_tile_member_getsetter::get_crit_descendant_nothrow(dst);
                                }
            ...
            struct InternalCudaResolutor: dg::network_producer_consumer::KVConsumerInterface<cuda_ptr_t, std::tuple<cuda_ptr_t, cuda_ptr_t, cuda_ptr_t, cuda_ptr_t, grad_status_t, cuda_tileops_dispatch_control_t>>{
                
                dg::network_cuda_controller::AsynchronousDeviceInterface * async_device;
                dg::network_cuda_controller::Synchronizer * synchronizer;
                dg::network_cuda_controller::RestrictPointerSynchronizer * restrict_synchronizer;

                void push(cuda_ptr_t, std::tuple<cuda_ptr_t, cuda_ptr_t, cuda_ptr_t, cuda_ptr_t, grad_status_t, cuda_tileops_dispatch_control_t> * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        auto [dst_logit_cudaptr, src_logit_cudaptr, dst_crit_cudaptr, dst_grad_cudaptr, grad_status, tileops_dispatch_control] = data_arr[i];
                        cuda_ptr_arr[i * 4]     = dst_logit_cudaptr;
                        cuda_ptr_arr[i * 4 + 1] = src_logit_cudaptr;
                        cuda_ptr_arr[i * 4 + 2] = dst_crit_cudaptr;
                        cuda_ptr_arr[i * 4 + 3] = dst_grad_cudaptr;

                        dg::network_tileops_cuda_poly::forward_crit_dispatch_chk(dst_logit_cudaptr, src_logit_cudaptr, dst_crit_cudaptr, dst_grad_cudaptr, tileops_dispatch_control); //it's better to split the responsibility here
                        total_complexity        += dg::network_tileops_cuda_poly::decode_crit_dispatch_control(tileops_dispatch_control)->runtime_complexity;
                        auto work_order         = [dst_logit_cudaptr, src_logit_cudaptr, dst_crit_cudaptr, dst_grad_cudaptr, grad_status, tileops_dispatch_control]() noexcept{
                            dg::network_exception_handler::nothrow_log(dg::network_tileops_cuda_poly::forward_crit(dst_logit_cudaptr, src_logit_cudaptr, dst_crit_cudaptr, dst_grad_cudaptr, tileops_dispatch_control)); //TODOs:
                        };
                        auto virtual_wo         = dg::network_cuda_controller::virtualize_async_task(std::move(work_order));
            ...
            struct InternalHostResolutor: dg::network_producer_consumer::KVConsumerInterface<host_ptr_t, std::tuple<host_ptr_t, host_ptr_t, host_ptr_t, host_ptr_t, grad_status_t, host_tileops_dispatch_control_t>>{

                dg::network_host_asynchronous::AsynchronousDeviceInterface * async_device;
                dg::network_host_asynchronous::Synchronizer * synchronizer;
                dg::network_host_asynchronous::RestrictPointerSynchronizer * restrict_synchronizer;

                void push(host_ptr_t, std::tuple<host_ptr_t, host_ptr_t, host_ptr_t, host_ptr_t, grad_status_t, host_tileops_dispatch_control_t> * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        auto [dst_logit_hostptr, src_logit_hostptr, dst_crit_hostptr, dst_grad_hostptr, grad_status, tileops_dispatch_control] = data_arr[i];
                        host_ptr_arr[i * 4]     = dst_logit_hostptr;
                        host_ptr_arr[i * 4 + 1] = src_logit_hostptr;
                        host_ptr_arr[i * 4 + 2] = dst_crit_hostptr;
                        host_ptr_arr[i * 4 + 3] = dst_grad_hostptr;

                        dg::network_tileops_host_poly::forward_crit_dispatch_chk(dst_logit_hostptr, src_logit_hostptr, dst_crit_hostptr, dst_grad_hostptr, tileops_dispatch_control); //it's better to split the responsibility here
                        total_complexity        += dg::network_tileops_host_poly::decode_crit_dispatch_control(tileops_dispatch_control)->runtime_complexity;
                        auto work_order         = [dst_logit_hostptr, src_logit_hostptr, dst_crit_hostptr, dst_grad_hostptr, grad_status, tileops_dispatch_control]() noexcept{
                            dg::network_exception_handler::nothrow_log(dg::network_tileops_host_poly::forward_crit(dst_logit_hostptr, src_logit_hostptr, dst_crit_hostptr, dst_grad_hostptr, tileops_dispatch_control)); //TODOs:
                        };
                        auto virtual_wo         = dg::network_host_asynchronous::virtualize_async_task(std::move(work_order));
                        virtual_wo_vec->add(std::move(virtual_wo));
                    }
            ...
            struct InternalResolutor: dg::network_producer_consumer::KVConsumerInterface<std::tuple<uma_ptr_t, uma_ptr_t>, std::tuple<uma_ptr_t, uma_ptr_t, operatable_id_t>>{
                ...
                void push(std::tuple<uma_ptr_t, uma_ptr_t> lck_addr, std::tuple<uma_ptr_t, uma_ptr_t, operatable_id_t> * data_arr, size_t sz) noexcept{
                    ...
                    auto umamap_reacquirer                          = dg::network_exception_handler::nothrow_log(dg::network_uma::reacquirer_fixedsize_raii_initialize(std::integral_constant<size_t, 4>{}));
                    auto dst_logit_vmamap_reacquirer                = dg::network_exception_handler::nothrow_log(dg::network_vmamap::reacquirer_raii_initialize());
                    auto src_logit_vmamap_reacquirer                = dg::network_exception_handler::nothrow_log(dg::network_vmamap::reacquirer_raii_initialize());
                    auto dst_crit_vmamap_reacquirer                 = dg::network_exception_handler::nothrow_log(dg::network_vmamap::reacquirer_raii_initialize());
                    ...
                    size_t trimmed_cuda_vectorization_sz            = std::min(this->vectorization_sz, sz);
                    size_t cdh_allocation_cost                      = dg::network_producer_consumer::delvrsrv_kv_allocation_cost(&cuda_internal_resolutor, trimmed_cuda_vectorization_sz);
                    dg::network_stack_allocation::NoExceptRawAllocation<char[]> cdh_mem(cdh_allocation_cost);
                    auto cuda_delivery_handle                       = dg::network_exception_handler::nothrow_log(dg::network_producer_consumer::delvsrv_open_kv_preallocated_raiihandle(&cuda_internal_resolutor, trimmed_cuda_vectorization_sz, cdh_mem.get()));

                    size_t trimmed_host_vectorization_sz            = std::min(this->vectorization_sz, sz);
                    size_t hdh_allocation_cost                      = dg::network_producer_consumer::delvrsrv_kv_allocation_cost(&host_internal_resolutor, trimmed_host_vectorization_sz);
                    dg::network_stack_allocation::NoExceptRawAllocation<char[]> hdh_mem(hdh_allocation_cost);
                    auto host_delivery_handle                       = dg::network_exception_handler::nothrow_log(dg::network_producer_consumer::delvsrv_open_kv_preallocated_raiihandle(&host_internal_resolutor, trimmed_host_vectorization_sz, hdh_mem.get()));
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        vma_ptr_t dst_logit_vmaptr  = dg::network_uma::get_vma_ptr(umamap_reacquirer, std::integral_constant<size_t, 0>{});
                        vma_ptr_t src_logit_vmaptr  = dg::network_uma::get_vma_ptr(umamap_reacquirer, std::integral_constant<size_t, 1>{});
                        vma_ptr_t dst_crit_vmaptr   = dg::network_uma::get_vma_ptr(umamap_reacquirer, std::integral_constant<size_t, 2>{});
                        vma_ptr_t dst_grad_vmaptr   = dg::network_uma::get_vma_ptr(umamap_reacquirer, std::integral_constant<size_t, 3>{});
                        ...
                        dg::network_vmamap::reacquirer_reacquire_nothrow(dst_logit_vmamap_reacquirer, dst_logit_vmaptr);
                        dg::network_vmamap::reacquirer_reacquire_nothrow(src_logit_vmamap_reacquirer, src_logit_vmaptr);
                        dg::network_vmamap::reacquirer_reacquire_nothrow(dst_crit_vmamap_reacquirer, dst_crit_vmaptr);
                        dg::network_vmamap::reacquirer_reacquire_nothrow(dst_grad_vmamap_reacquirer, dst_grad_vmaptr);

                        if (dg::network_dispatch_control::is_cuda_dispatch(dispatch_info.dispatch_platform)){
                            cuda_ptr_t dst_logit_cudaptr    = dg::network_vmamap::get_cuda_ptr(dst_logit_vmamap_reacquirer);
                            cuda_ptr_t src_logit_cudaptr    = dg::network_vmamap::get_cuda_ptr(src_logit_vmamap_reacquirer);
                            cuda_ptr_t dst_crit_cudaptr     = dg::network_vmamap::get_cuda_ptr(dst_crit_vmamap_reacquirer);
                            cuda_ptr_t dst_grad_cudaptr     = dg::network_vmamap::get_cuda_ptr(dst_grad_vmamap_reacquirer);

                            dg::network_producer_consumer::delvrsrv_deliver(cuda_delivery_handle.get(), src_logit_cudaptr, std::make_tuple(dst_logit_cudaptr, src_logit_cudaptr, dst_crit_cudaptr, dst_grad_cudaptr, dst_grad_status, dispatch_info.tileops_cuda_dispatch_control));
                        } else if (dg::network_dispatch_control::is_host_dispatch(dispatch_info.dispatch_platform)){
                            host_ptr_t dst_logit_hostptr    = dg::network_vmamap::get_host_ptr(dst_logit_vmamap_reacquirer);
                            host_ptr_t src_logit_hostptr    = dg::network_vmamap::get_host_ptr(src_logit_vmamap_reacquirer);
                            host_ptr_t dst_crit_hostptr     = dg::network_vmamap::get_host_ptr(dst_crit_vmamap_reacquirer);
                            host_ptr_t dst_grad_hostptr     = dg::network_vmamap::get_host_ptr(dst_grad_vmamap_reacquirer);

                            dg::network_producer_consumer::delvrsrv_deliver(host_delivery_handle.get(), src_logit_hostptr, std::make_tuplke(dst_logit_hostptr, src_logit_hostptr, dst_crit_hostptr, dst_grad_hostptr, dst_grad_status, dispatch_info.tileops_host_dispatch_control));
                        } else{
                            if constexpr(DEBUG_MODE_FLAG){
                                dg::network_log_stackdump::critical(dg::network_exception::verbose(dg::network_exception::INTERNAL_CORRUPTION));
                                std::abort();
            ...
            struct InternalHostResolutor: dg::network_producer_consumer::KVConsumerInterface<host_ptr_t, std::tuple<host_ptr_t, host_ptr_t, host_ptr_t, host_tileops_dispatch_control_t>>{
                ...
                void push(host_ptr_t, std::tuple<host_ptr_t, host_ptr_t, host_ptr_t, host_tileops_dispatch_control_t> * data_arr, size_t sz) noexcept{ //we are ditching this ambiguity later
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        host_ptr_vec[i * 3 + 1]     = src;
                        host_ptr_vec[i * 3 + 2]     = cpy_dst;
                        dg::network_exception_handler::nothrow_log(dg::network_tileops_host_poly::forward_mono_dispatch_chk(dst, src, tileops_dispatch_control)); //we already guard the dispatch_control
                        total_complexity            += dg::network_tileops_host_poly::decode_mono_dispatch_control(tileops_dispatch_control)->runtime_complexity; //this is unsafe access of std::expected - which is fine - std has internal mechanism to guard this
                        total_complexity            += dg::network_tileops_host_poly::decode_mono_dispatch_control(MEMCPY)->runtime_complexity;
                        auto work_order             = [dst, src, cpy_dst, tileops_dispatch_control]() noexcept{
                            dg::network_exception_handler::nothrow_log(dg::network_tileops_host_poly::forward_mono(dst, src, tileops_dispatch_control));
                            dg::network_exception_handler::nothrow_log(dg::network_memops::memcpy_host_to_host(cpy_dst, dst, dg::network_tileops_host_poly::decode_mono_dispatch_control(tileops_dispatch_control)->byte_size));
                        };
                        auto virtual_wo             = dg::network_host_asynchronous::virtualize_async_task(std::move(work_order)); 
                        dg::network_exception_handler::nothrow_log(virtual_wo_vec->add(std::move(virtual_wo)));
                    }
                ...
                void push(std::tuple<uma_ptr_t, uma_ptr_t> lck_addr, std::tuple<uma_ptr_t, uma_ptr_t, operatable_id_t> * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        if (dg::network_dispatch_control::is_host_dispatch(dispatch_info.dispatch_platform)){ //we are doing cuda_ptr_t (cutf_ptr_t) -> host_ptr_t
                            ...
                            msgrfwd_data.logit_value        = dg::string(dg::network_tileops_host_poly::get_byte_size(dispatch_info.tileops_host_dispatch_control), ' '); //we want raw strings
                            ...
                            dg::network_producer_consumer::delvrsrv_deliver(host_vectorizer.get(), src_logit_hostptr, std::make_tuple(dst_logit_hostptr, src_logit_hostptr, logit_value_data, dispatch_info.tileops_host_dispatch_control));
                        } else{
                            if constexpr(DEBUG_MODE_FLAG){
                                dg::network_log_stackdump::critical(dg::network_exception::verbose(dg::network_exception::INTERNAL_CORRUPTION));
                                std::abort();
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        eu_packet.content               = dg::network_compact_serializer::serialize<dg::string>(LogitValue{msgrfwd_outbound_vec[i]->addr, std::move(msgrfwd_outbound_vec[i]->logit_value)}); //pollute hardware cache
                        ...
                        eu_packet.exception_handler     = dg::network_exception::make_exception_handler_from_lambda([user_id, addr](exception_t err) noexcept{
                            if (dg::network_exception::is_failed(err)){
    ...
    class ForwardDoMsgrBwdSignalResolutor: public virtual dg::network_producer_consumer::ConsumerInterface<ForwardDoSignalEvent>{
        ...
        private:
            ...
            struct InternalCudaResolutor: dg::network_producer_consumer::KVConsumerInterface<cuda_ptr_t, std::tuple<cuda_ptr_t, cuda_ptr_t, cuda_tileops_dispatch_control_t>>{
                ...
                void push(cuda_ptr_t, std::tuple<cuda_ptr_t, cuda_ptr_t, cuda_tileops_dispatch_control_t> * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        total_complexity        += dg::network_tileops_cuda_poly::decode_mono_dispatch_control(tileops_dispatch_control)->runtime_complexity;
                        auto work_order         = [dst, src, tileops_dispatch_control]() noexcept{
                            dg::network_exception_handler::nothrow_log(dg::network_tileops_cuda_poly::forward_mono(dst, src, tileops_dispatch_control)); //alright - I will fix the cuda later - but I hardly seen cuda crashed - I've been playing games for years - this might not be a reasonable exit strategy
                        };
                        auto virtual_wo         = dg::network_cuda_controller::virtualize_async_task(std::move(work_order));
                        dg::network_exception_handler::nothrow_log(virtual_wo_vec->add(std::move(virtual_wo)));
                    }
            ...
            struct InternalHostResolutor: dg::network_producer_consumer::KVConsumerInterface<host_ptr_t, std::tuple<host_ptr_t, host_ptr_t, cuda_tileops_dispatch_control_t>>{
            ...
            struct InternalResolutor: dg::network_producer_consumer::KVConsumerInterface<std::tuple<uma_ptr_t, uma_ptr_t>, std::tuple<uma_ptr_t, uma_ptr_t, operatable_id_t>>{
                ...
                void push(std::tuple<uma_ptr_t, uma_ptr_t> lck_addr, std::tuple<uma_ptr_t, uma_ptr_t, operatable_id_t> * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        if (!dg::network_uma::reacquirer_fixedsize_is_region_reacquirable(umamap_reacquirer, {{dst_logit_umaptr, dispatch_info.dst_vd_id}, 
                                                                                                              {src_logit_umaptr.value(), dispatch_info.src_vd_id}})){ //we assume the semantics of reacquirable == reachable vma_ptr_t(s) are unaffected - this needs to be documented
                        ...
                        if (!dg::network_vmamap::reacquirer_is_region_reacquirable(dst_vmamap_reacquirer, dst_vmaptr) 
                            || !dg::network_vmamap::reacquirer_is_region_reacquirable(src_vmamap_reacquirer, src_vmaptr)){
                        ...
                        dg::network_vmamap::reacquirer_reacquire_nothrow(dst_vmamap_reacquirer, dst_vmaptr);
                        dg::network_vmamap::reacquirer_reacquire_nothrow(src_vmamap_reacquirer, src_vmaptr);
                        ...
                        if (dg::network_dispatch_control::is_cuda_dispatch(dispatch_info.dispatch_platform)){
                            ...
                            dg::network_producer_consumer::delvrsrv_deliver(cuda_delivery_handle.get(), src_cuda_ptr, std::make_tuple(dst_cuda_ptr, src_cuda_ptr, dispatch_info.tileops_cuda_dispatch_control));
                        } else if (dg::network_dispatch_control::is_host_dispatch(dispatch_info.dispatch_platform)){
                            ...
                            dg::network_producer_consumer::delvrsrv_deliver(host_delivery_handle.get(), src_host_ptr, std::make_tuple(dst_host_ptr, src_host_ptr, dispatch_info.tileops_host_dispatch_control));
                        } else{
                            if constexpr(DEBUG_MODE_FLAG){
    ...
    class BackwardDoLeafSignalResolutorV2: public virtual dg::network_producer_consumer::ConsumerInterface<BackwardDoSignalEvent>{
        ...
        public:
            ...
            void push(BackwardDoSignalEvent * event_arr, size_t sz) noexcept{
                ...
                size_t trimmed_region_vectorization_sz  = std::min(this->region_vectorization_sz, sz);
                size_t dh_allocation_cost               = dg::network_producer_consumer::delvrsrv_kv_allocation_cost(&internal_resolutor, trimmed_region_vectorization_sz);
                dg::network_stack_allocation::NoExceptRawAllocation<char[]> dh_mem(dh_allocation_cost);
        ...
        private:
            ...
            struct InternalResolutor: dg::network_producer_consumer::KVConsumerInterface<uma_ptr_t, std::tuple<uma_ptr_t, operatable_id_t>>{
                ...
                void push(uma_ptr_t rcu_addr, std::tuple<uma_ptr_t, operatable_id_t> * data_arr, size_t sz) noexcept{
                    ...
                    auto umamap_reacquirer          = dg::network_exception_handler::nothrow_log(dg::network_uma::reacquirer_fixedsize_raii_initialize(std::integral_constant<size_t, 2u>{}));
                    auto logit_vmamap_reacquirer    = dg::network_exception_handler::nothrow_log(dg::network_vmamap::reacquirer_raii_initialize());
                    auto grad_vmamap_reacquirer     = dg::network_exception_handler::nothrow_log(dg::network_vmamap::reacquirer_raii_initialize());
                    auto cuda_synchronizer          = dg::network_exception_handler::nothrow_log(dg::network_exception::cstyle_initialize<dg::network_cuda_controller::Synchronizer>());
                    auto host_synchronizer          = dg::network_exception_handler::nothrow_log(dg::network_exception::cstyle_initialize<dg::network_host_asynchronous::Synchronizer>());
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        auto dispatch_info = dg::network_exception_handler::nothrow_log(dg::network_dispatch_control::decode_leaf_backward_dispatch(dispatch_control));
                        ...
                        dg::network_uma::reacquirer_fixedsize_reacquire_nothrow(umamap_reacquirer, {{logit_umaptr, dispatch_info.logit_vd_id}, 
                                                                                                    {grad_umaptr, dispatch_info.grad_vd_id}});
    ...
    class BackwardDoMonoSignalResolutorV2: public virtual dg::network_producer_consumer::ConsumerInterface<BackwardDoSignalEvent>{
            ...
            void push(BackwardDoSignalEvent * event_arr, size_t sz) noexcept{
...

                {
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        std::expected<uma_ptr_t, exception_t> descendant_rcu_addr = dg::network_tile_member_getsetter::get_tile_rcu_addr(descendant_arr[i].value());
                        ...
                        uma_ptr_t dst_rcu_addr          = dg::network_tile_member_getsetter::get_mono_rcu_addr_nothrow(event_arr[i].dst);
                        uma_ptr_t dst_lck_addr          = dg::memult::region(dst_rcu_addr, lck_region_sz);
                        uma_ptr_t src_lck_addr          = dg::memult::region(descendant_rcu_addr.value(), lck_region_sz);
                        auto key                        = std::make_tuple(dst_lck_addr, src_lck_addr);
        ...
        private:
                ...
                void push(uma_ptr_t rcu_addr, AddressFetcherArgument * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        switch (init_status){
                            ...
                            case TILE_INIT_STATUS_DECAYED:
                            {
                                *data_arr[i].fetching_addr = std::nullopt; //
                                break;
                            }
                            case TILE_INIT_STATUS_INITIALIZED:
                            {
                                if (data_arr[i].expected_ops_id != current_ops_id){
                                    ...
                                    *data_arr[i].fetching_addr = dg::network_tile_member_getsetter::get_mono_descendant_nothrow(data_arr[i].root); //
                                }
            ...
            struct InternalCudaResolutor: dg::network_producer_consumer::KVConsumerInterface<cuda_ptr_t, CudaResolutorArgument>{
                dg::network_cuda_controller::AsynchronousDeviceInterface * async_device;
                dg::network_cuda_controller::Synchronizer * synchronizer;
                dg::network_cuda_controller::RestrictPointerSynchronizer * restrict_synchronizer;

                void push(cuda_ptr_t, CudaResolutorArgument * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        dg::network_exception_handler::nothrow_log(dg::network_tileops_cuda_poly::backward_mono_dispatch_chk(data_arr[i].dispatch_control));
                        total_complexity        += dg::network_tileops_cuda_poly::decode_mono_backward_dispatch(data_arr[i].dispatch_control)->runtime_complexity;
                        auto work_order         = [e = data_arr[i]]() noexcept{
                            dg::network_exception_handler::nothrow_log(dg::network_tileops_cuda_poly::backward_mono(e.src_grad_addr, e.src_logit_addr, e.dst_grad_addr, e.dispatch_control)); //TODOs:
                        };
                        auto virtual_wo         = dg::network_cuda_controller::virtualize_async_task(std::move(work_order));
                        dg::network_exception_handler::nothrow_log(virtual_wo_vec->add(std::move(virtual_wo)));
                    }
            ...
            struct InternalHostResolutor: dg::network_producer_consumer::KVConsumerInterface<host_ptr_t, HostResolutorArgument>{
                dg::network_host_asynchronous::AsynchronousDeviceInterface * async_device;
                dg::network_host_asynchronous::Synchronizer * synchronizer;
                dg::network_host_asynchronous::RestrictPointerSynchronizer * restrict_synchronizer;

                void push(host_ptr_t, HostResolutorArgument * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        dg::network_exception_handler::nothrow_log(dg::network_tileops_host_poly::backward_mono_dispatch_chk(data_arr[i].dispatch_control));
                        total_complexity        += dg::network_tileops_host_poly::decode_mono_backward_dispatch(data_arr[i].dispatch_control)->runtime_complexity;
                        auto work_order         = [e = data_arr[i]]() noexcept{
                            dg::network_exception_handler::nothrow_log(dg::network_tileops_host_poly::backward_mono(e.src_grad_addr, e.src_logit_addr, e.dst_grad_addr, e.dispatch_control)); //TODOs:
                        };
                        auto virtual_wo         = dg::network_host_asynchronous::virtualize_async_task(std::move(work_order));
                        dg::network_exception_handler::nothrow_log(virtual_wo_vec->add(std::move(virtual_wo)));
                    }
            ...
            struct InternalResolutor: dg::network_producer_consumer::KVConsumerInterface<ResolutorKeyArgument, ResolutorValueArgument>{
                ...
                void push(ResolutorKeyArgument key, ResolutorValueArgument * data_arr, size_t sz) noexcept{
                    ...
                    auto umamap_reacquirer                              = dg::network_exception_handler::nothrow_log(dg::network_uma::reacquirer_fixedsize_raii_initialize(std::integral_constant<size_t, 3u>{}));
                    auto src_grad_vmamap_reacquirer                     = dg::network_exception_handler::nothrow_log(dg::network_vmamap::reacquirer_raii_initialize());
                    auto src_logit_vmamap_reacquirer                    = dg::network_exception_handler::nothrow_log(dg::network_vmamap::reacquirer_raii_initialize());
                    auto dst_grad_vmamap_reacquirer                     = dg::network_exception_handler::nothrow_log(dg::network_vmamap::reacquirer_raii_initialize());
                    ...
                    size_t cdh_allocation_cost                          = dg::network_producer_consumer::delvrsrv_kv_allocation_cost(&cuda_internal_resolutor, trimmed_cuda_vectorization_sz);
                    dg::network_stack_allocation::NoExceptRawAllocation<char[]> cdh_mem(cdh_allocation_cost);
                    auto cuda_delivery_handle                           = dg::network_exception_handler::nothrow_log(dg::network_producer_consumer::delvrsrv_open_kv_preallocated_raiihandle(&cuda_internal_resolutor, trimmed_cuda_vectorization_sz, cdh_mem.get()));

                    size_t trimmed_host_vectorization_sz                = std::min(this->vectorization_sz, sz);
                    size_t hdh_allocation_cost                          = dg::network_producer_consumer::delvrsrv_kv_allocation_cost(&host_internal_resolutor, trimmed_host_vectorization_sz);
                    dg::network_stack_allocation::NoExceptRawAllocation<char[]> hdh_mem(hdh_allocation_cost);
                    auto host_delivery_handle                           = dg::network_exception_handler::nothrow_log(dg::network_producer_consumer::delvrsrv_open_kv_preallocated_raiihandle(&host_internal_resolutor, trimmed_host_vectorization_sz, hdh_mem.get()));
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        auto dispatch_info = dg::network_exception_handler::nothrow_log(dg::network_dispatch_control::decode_mono_backward_control(dispatch_control));
                        ...
                        dg::network_uma::reacquirer_reacquire_nothrow(umamap_reacquirer, {{src_grad_umaptr.value(), dispatch_info.src_grad_vd_id},
                                                                                          {src_logit_umaptr.value(), dispatch_info.src_logit_vd_id},
                                                                                          {dst_grad_umaptr, dispatch_info.dst_grad_id_id}});
    ...
    class BackwardDoPairSignalResolutorV2: public virtual dg::network_producer_consumer::ConsumerInterface<BackwardDoSignalEvent>{
            ...
            void push(BackwardDoSignalEvent * event_arr, size_t sz) noexcept{
...

                {
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        auto key                = std::make_tuple(dst_lck_addr, lhs_lck_addr, rhs_lck_addr); 

                        dg::network_producer_consumer::delvrsrv_deliver(vectorized_delivery_handle.get(), key, std::make_tuple(event_arr[i].dst, lhs_ptr, rhs_ptr, event_arr[i].operatable_id));
                    }
                }
            }
        ...
        private:
            ...
            //we'll come up with the naming convention later
            //we'll be adding readability by using named structs later 

            struct InternalCudaMixedResolutor: dg::network_producer_consumer::ConsumerInterface<std::tuple<cuda_ptr_t, cuda_ptr_t, cuda_ptr_t, cuda_ptr_t, cuda_ptr_t, cuda_tileops_dispatch_control_t>>{

                dg::network_cuda_controller::AsynchronousDeviceInterface * async_device;
                dg::network_cuda_controller::Synchronizer * synchronizer;
                dg::network_cuda_controller::RestrictPointerSynchronizer * restrict_synchronizer;

                void push(std::tuple<cuda_ptr_t, cuda_ptr_t, cuda_ptr_t, cuda_ptr_t, cuda_ptr_t, cuda_tileops_dispatch_control_t> * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        auto [lhs_logit_ptr, lhs_grad_ptr, rhs_logit_ptr, rhs_grad_ptr, dst_grad_ptr, tileops_dispatch_control] = data_arr[i];
                        cuda_ptr_arr[i * 5]         = lhs_logit_ptr;
                        cuda_ptr_arr[i * 5 + 1]     = lhs_grad_ptr;
                        cuda_ptr_arr[i * 5 + 2]     = rhs_logit_ptr;
                        cuda_ptr_arr[i * 5 + 3]     = rhs_grad_ptr;
                        cuda_ptr_arr[i * 5 + 4]     = dst_grad_ptr;
                        dg::network_exception_handler::nothrow_log(dg::network_tileops_cuda_poly::backward_pair_dispatch_chk(lhs_logit_ptr, lhs_grad_ptr, rhs_logit_ptr, rhs_grad_ptr, dst_grad_ptr, tileops_dispatch_control));
                        total_complexity            += dg::network_tileops_cuda_poly::decode_backward_pair_dispatch_control(tileops_dispatch_control)->runtime_complexity;
                        auto work_order             = [lhs_logit_ptr, lhs_grad_ptr, rhs_logit_ptr, rhs_grad_ptr, dst_grad_ptr, tileops_dispatch_control]() noexcept{
                            dg::network_exception_handler::nothrow_log(dg::network_tileops_cuda_poly::backward_pair(lhs_logit_ptr, lhs_grad_ptr, rhs_logit_ptr, rhs_grad_ptr, dst_grad_ptr, tileops_dispatch_control));
                        };
                        auto virtual_wo             = dg::network_cuda_controller::virtualize_async_task(std::move(work_order));
                        dg::network_exception_handler::nothrow_log(virtual_wo_vec->add(std::move(virtual_wo))); 
                    }
            ...
            struct InternalCudaResolutor: dg::network_producer_consumer::KVConsumerInterface<cuda_ptr_t, std::tuple<cuda_ptr_t, cuda_ptr_t, cuda_ptr_t, cuda_ptr_t, cuda_ptr_t, cuda_tileops_dispatch_control_t>>{

                dg::network_cuda_controller::AsynchronousDeviceInterface * async_device;
                dg::network_cuda_controller::Synchronizer * synchronizer;
                dg::network_cuda_controller::RestrictPointerSynchronizer * restrict_synchronizer;

                void push(cuda_ptr_t, std::tuple<cuda_ptr_t, cuda_ptr_t, cuda_ptr_t, cuda_ptr_t, cuda_ptr_t, cuda_tileops_dispatch_control_t> * data_arr, size_t sz) noexcept{
            ...
            struct InternalHostMixedResolutor: dg::network_producer_consumer::ConsumerInterface<std::tuple<host_ptr_t, host_ptr_t, host_ptr_t, host_ptr_t, host_ptr_t, host_tileops_dispatch_control_t>>{
                ...
                void push(std::tuple<host_ptr_t, host_ptr_t, host_ptr_t, host_ptr_t, host_ptr_t, cuda_tileops_dispatch_control_t> * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        auto [lhs_logit_ptr, lhs_grad_ptr, rhs_logit_ptr, rhs_grad_ptr, dst_grad_ptr, tileops_dispatch_control] = data_arr[i];
                        host_ptr_arr[i * 5]         = lhs_logit_ptr;
                        host_ptr_arr[i * 5 + 1]     = lhs_grad_ptr;
                        host_ptr_arr[i * 5 + 2]     = rhs_logit_ptr;
                        host_ptr_arr[i * 5 + 3]     = rhs_grad_ptr;
                        host_ptr_arr[i * 5 + 4]     = dst_grad_ptr;
                        dg::network_exception_handler::nothrow_log(dg::network_tileops_host_poly::backward_pair_dispatch_chk(lhs_logit_ptr, lhs_grad_ptr, rhs_logit_ptr, rhs_grad_ptr, dst_grad_ptr, tileops_dispatch_control));
                        total_complexity            += dg::network_tileops_host_poly::decode_backward_pair_dispatch_control(tileops_dispatch_control)->runtime_complexity;
                        auto work_order             = [lhs_logit_ptr, lhs_grad_ptr, rhs_logit_ptr, rhs_grad_ptr, dst_grad_ptr, tileops_dispatch_control]() noexcept{
                            dg::network_exception_handler::nothrow_log(dg::network_tileops_host_poly::backward_pair(lhs_logit_ptr, lhs_grad_ptr, rhs_logit_ptr, rhs_grad_ptr, dst_grad_ptr, tileops_dispatch_control));
                        };
                        auto virtual_wo             = dg::network_host_asynchronous::virtualize_async_task(std::move(work_order));
                        dg::network_exception_handler::nothrow_log(virtual_wo_vec->add(std::move(virtual_wo)));
                    }
            ...
            struct InternalHostResolutor: dg::network_producer_consumer::KVConsumerInterface<host_ptr_t, std::tuple<host_ptr_t, host_ptr_t, host_ptr_t, host_ptr_t, host_tileops_dispatch_control_t>>{

                dg::network_host_asynchronous::AsynchronousDeviceInterface * async_device;
                dg::network_host_asynchronous::Synchronizer * synchronizer;
                dg::network_host_asynchronous::RestrictPointerSynchronizer * restrict_synchronizer;

                void push(host_ptr_t, std::tuple<host_ptr_t, host_ptr_t, host_ptr_t, host_ptr_t, host_tileops_dispatch_control_t> * data_arr, size_t sz) noexcept{

                    auto base_resolutor                     = InternalHostMixedResolutor{};
                    base_resolutor.async_device             = this->async_device;
                    base_resolutor.synchronizer             = this->synchronizer;
                    base_resolutor.restrict_synchronzier    = this->restrict_synchronizer;
                    base_resolutor.push(data_arr, sz);
                }
            };
            ...
            struct InternalResolutor: dg::network_producer_consumer::KVConsumerInterface<std::tuple<uma_ptr_t, uma_ptr_t, uma_ptr_t>, std::tuple<uma_ptr_t, uma_ptr_t, uma_ptr_t, operatable_id_t>>{
                ...
                void push(std::tuple<uma_ptr_t, uma_ptr_t, uma_ptr_t> lck_addr, std::tuple<uma_ptr_t, uma_ptr_t, uma_ptr_t, operatable_id_t> * data_arr, size_t sz) noexcept{
                    ...
                    auto umamap_reacquirer                              = dg::network_exception_handler::nothrow_log(dg::network_uma::reacquirer_fixedsize_raii_initialize(std::integral_constant<size_t, 5u>{}));
                    auto dst_grad_vmamap_reacquirer                     = dg::network_exception_handler::nothrow_log(dg::network_vmamap::reacquirer_raii_initialize());
                    auto lhs_grad_vmamap_reacquirer                     = dg::network_exception_handler::nothrow_log(dg::network_vmamap::reacquirer_raii_initialize());
                    auto lhs_logit_vmamap_reacquirer                    = dg::network_exception_handler::nothrow_log(dg::network_vmamap::reacquirer_raii_initialize());
                    auto rhs_grad_vmamap_reacquirer                     = dg::network_exception_handler::nothrow_log(dg::network_vmamap::reacquirer_raii_initialize());
                    auto rhs_logit_vmamap_reacquirer                    = dg::network_exception_handler::nothrow_log(dg::network_vmamap::reacquirer_raii_initialize());
                    ...
                    auto cuda_mixed_internal_resolutor                  = InternalCudaMixedResolutor{};
                    cuda_mixed_internal_resolutor.async_device          = this->cuda_async_device;
                    cuda_mixed_internal_resolutor.synchronizer          = &cuda_synchronizer;
                    cuda_mixed_internal_resolutor.restrict_synchronizer = &cuda_restrict_synchronizer;

                    auto host_mixed_internal_resolutor                  = InternalHostMixedResolutor{};
                    host_mixed_internal_resolutor.async_device          = this->host_async_device;
                    host_mixed_internal_resolutor.synchronizer          = &host_synchronizer;
                    host_mixed_internal_resolutor.restrict_synchronizer = &host_restrict_synchronizer;
                    ...
                    size_t trimmed_cuda_mixed_vectorization_sz          = std::min(this->vectorization_sz, sz);
                    size_t vmdh_allocation_cost                         = dg::network_producer_consumer::delvrsrv_allocation_cost(&cuda_mixed_internal_resolutor, trimmed_cuda_mixed_vectorization_sz);
                    dg::network_stack_allocation::NoExceptRawAllocation<char[]> vmdh_mem(vmdh_allocation_cost);
                    auto cuda_mixed_delivery_handle                     = dg::network_exception_handler::nothrow_log(dg::network_producer_consumer::delvrsrv_open_preallocated_raiihandle(&cuda_mixed_internal_resolutor, trimmed_cuda_mixed_vectorization_sz, vmdh_mem.get()));

                    size_t trimmed_host_mixed_vectorization_sz          = std::min(this->vectorization_sz, sz);
                    size_t hmdh_allocation_cost                         = dg::network_producer_consumer::delvrsrv_allocation_cost(&host_mixed_internal_resolutor, trimmed_host_mixed_vectorization_sz);
                    dg::network_stack_allocation::NoExceptRawAllocation<char[]> hmdh_mem(hmdh_allocation_cost);
                    auto host_mixed_delivery_handle                     = dg::network_exception_handler::nothrow_log(dg::network_producer_consumer::delvrsrv_open_preallocated_raiihandle(&host_mixed_internal_resolutor, trimmed_host_mixed_vectorization_sz, hmdh_mem.get())); 
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        if (!dg::network_uma::reacquirer_is_reacquirable(umamap_reacquirer, {{lhs_logit_umaptr.value(), dispatch_info.lhs_logit_vd_id},
                        ...
                        if (dg::network_dispatch_control::is_cuda_dispatch(dispatch_info.dispatch_platform)){
                            ...
                            if (dst_dispatch_major == PAIR_DISPATCH_MAJOR_RIGHT){
                                ...
                                dg::network_producer_consumer::delvrsrv_deliver(cuda_delivery_handle.get(), lhs_logit_cudaptr, work_order);
                            } else if (dst_dispatch_major == PAIR_DISPATCH_MAJOR_MIXED){
                                dg::network_producer_consumer::delvrsrv_deliver(cuda_mixed_delivery_handle.get(), work_order);
                            } else{
                                if constexpr(DEBUG_MODE_FLAG){
                                    dg::network_log_stackdump::critical(dg::network_exception::verbose(dg::network_exception::INTERNAL_CORRUPTION));
                                    std::abort();
                            ...
                            if (dst_dispatch_major == PAIR_DISPATCH_MAJOR_RIGHT){
                                ...
                                dg::network_producer_consumer::delvrsrv_deliver(host_delivery_handle.get(), lhs_logit_hostptr, work_order);
                            } else if (dst_dispatch_major == PAIR_DISPATCH_MAJOR_MIXED){
                                dg::network_producer_consumer::delvrsrv_deliver(host_mixed_delivery_handle.get(), work_order);
                            } else{
                                if constexpr(DEBUG_MODE_FLAG){
                                    dg::network_log_stackdump::critical(dg::network_exception::verbose(dg::network_exception::INTERNAL_CORRUPTION));
                                    std::abort();
                ...
                void push(uma_ptr_t rcu_addr, AddressFetchArgument * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        switch (init_status){
                            ...
                            case TILE_INIT_STATUS_DECAYED:
                            {
                                *data_arr[i].fetching_validation_addr = false;
                                break;
                            }
                            case TILE_INIT_STATUS_INITIALIZED:
                            {
    ...
    class BackwardDoExtnSrcSignalResolutorV2: public virtual dg::network_produer_consumer::ConsumerInterface<BackwardDoSignalEvent>{
        ...
        public:
            ...
            void push(BackwardDoSignalEvent * event_arr, size_t sz) noexcept{
                ...
                dg::network_stack_allocation::NoExceptAllocation<std::optional<std::tuple<uma_ptr_t, uma_ptr_t>>[]> descendant_localcounterpart_arr(sz);

                const size_t EVENT_SCALE_FACTOR         = 1u;
                size_t max_possible_event_sz            = sz * EVENT_SCALE_FACTOR;
                size_t trimmed_delivery_capacity        = std::min(this->request_delivery_capacity, max_possible_event_sz);
                size_t dh_allocation_cost               = dg::network_producer_consumer::delvrsrv_allocation_cost(this->request_box.get(), trimmed_delivery_capacity);
                dg::network_stack_allocation::NoExceptRawAllocation<char[]> dh_mem(dh_allocation_cost);
                auto request_delivery_handle            = dg::network_exception_handler::nothrow_log(dg::network_producer_consumer::delvrsrv_open_preallocated_raiihandle(this->request_box.get(), trimmed_delivery_capacity, dh_mem.get()));

                {
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        dg::network_producer_consumer::delvrsrv_deliver(vectorized_delivery_handle.get(), lck_addr, std::make_tuple(event_arr[i].dst, event_arr[i].operatable_id, std::next(descendant_localcounterpart_arr.get(), i)));
                    }
                }

                {
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        if (!descendant_rcu_addr.has_value() || !local_counterpart_rcu_addr.has_value()){
                            continue;                            
                        }
                        ...
                        auto key                            = std::make_tuple(descendant_lck_addr, dst_lck_addr, localcounterpart_lck_addr); 

                        dg::network_producer_consumer::delvrsrv_deliver(vectorized_delivery_handle.get(), key, std::make_tuple(descendant, event_arr[i].dst, local_counterpart, event_arr[i].operatable_id));
                    }
                }
            }
        ...
        private:
            ...
            struct InternalDescendantAddressFetcher: dg::network_producer_consumer::KVConsumerInterface<uma_ptr_t, std::tuple<uma_ptr_t, operatable_id_t, std::optional<std::tuple<uma_ptr_t, uma_ptr_t>> *>>{ //fix semantics - tuple is loosely defined - not clear

                ForeignTileAliasGetterInterface * alias_getter;

                void push(uma_ptr_t rcu_addr, std::tuple<uma_ptr_t, operatable_id_t, std::optional<std::tuple<uma_ptr_t, uma_ptr_t>> *> * data_arr, size_t sz){
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        switch (init_status){
                            ...
                            case TILE_INIT_STATUS_INITIALIZED:
                            {
                                ...
                                if (!local_counterpart.has_value() || !local_counterpart.value().has_value()){
                                    *fetching_addr = std::nullopt;
                                } else{
            ...
            struct InternalCudaResolutor: dg::network_producer_consumer::KVConsumerInterface<cuda_ptr_t, std::tuple<cuda_ptr_t, cuda_ptr_t, cuda_ptr_t, cuda_tileops_dispatch_control_t, grad_status_t>>{
                ...
                void push(cuda_ptr_t, std::tuple<cuda_ptr_t, cuda_ptr_t, cuda_ptr_t, cuda_tileops_dispatch_control_t, grad_status_t> * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        auto [src_grad_cudaptr, src_logit_cudaptr, dst_grad_cudaptr, tileops_dispatch_control, src_grad_status] = data_arr[i];
                        cuda_ptr_arr[i * 3]     = src_grad_cudaptr;
                        cuda_ptr_arr[i * 3 + 1] = src_logit_cudaptr;
                        cuda_ptr_arr[i * 3 + 2] = dst_grad_cudaptr;

                        dg::network_exception_handler::nothrow_log(dg::network_tileops_cuda_poly::backward_mono_dispatch_chk(src_grad_cudaptr, src_logit_cudaptr, dst_grad_cudaptr, tileops_dispatch_control));
                        total_complexity        += dg::network_tileops_cuda_poly::decode_mono_dispatch_control(tileops_dispatch_control)->runtime_complexity;
                        auto work_order         = [src_grad_cudaptr, src_logit_cudaptr, dst_grad_cudaptr, tileops_dispatch_control, src_grad_status]() noexcept{
                            dg::network_exception_handler::nothrow_log(dg::network_tileops_cuda_poly::backward_mono());
                        };
                        auto virtual_wo         = dg::network_cuda_controller::virtualize_async_task(std::move(work_order));
                        dg::network_exception_handler::nothrow_log(virtual_wo_vec->add(std::move(virtual_wo))); 
                    }
            ...
            struct InternalHostResolutor: dg::network_producer_consumer::KVConsumerInterface<host_ptr_t, std::tuple<host_ptr_t, host_ptr_t, host_ptr_t, host_tileops_dispatch_control_t, grad_status_t>>{
                ...
                void push(host_ptr_t, std::tuple<host_ptr_t, host_ptr_t, host_ptr_t, host_tileops_dispatch_control_t, grad_status_t> * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        auto [src_grad_hostptr, src_logit_hostptr, dst_grad_hostptr, tileops_dispatch_control, src_grad_status] = data_arr[i];
                        host_ptr_arr[i * 3]     = src_grad_hostptr;
                        host_ptr_arr[i * 3 + 1] = src_logit_hostptr;
                        host_ptr_arr[i * 3 + 2] = dst_grad_hostptr;

                        dg::network_exception_handler::nothrow_log(dg::network_tileops_host_poly::backward_mono_dispatch_chk(src_grad_hostptr, src_logit_hostptr, dst_grad_hostptr, tileops_dispatch_control));
                        total_complexity        += dg::network_tileops_host_poly::decode_mono_dispatch_control(tileops_dispatch_control)->runtime_complexity;
                        auto work_order         = [src_grad_hostptr, src_logit_hostptr, dst_grad_hostptr, tileops_dispatch_control, src_grad_status]() noexcept{
                            dg::network_exception_handler::nothrow_log(dg::network_tileops_host_poly::backward_mono());
                        };
                        auto virtual_wo         = dg::network_host_asynchronous::virtualize_async_task(std::move(work_order));
                        dg::network_exception_handler::nothrow_log(virtual_wo_vec->add(std::move(virtual_wo)));
                    }
            ...
            struct InternalResolutor: dg::network_producer_consumer::KVConsumerInterface<std::tuple<uma_ptr_t, uma_ptr_t, uma_ptr_t>, std::tuple<uma_ptr_t, uma_ptr_t, uma_ptr_t, operatable_id_t>>{
                ...
                void push(std::tuple<uma_ptr_t, uma_ptr_t, uma_ptr_t> lck_addr, std::tuple<uma_ptr_t, uma_ptr_t, uma_ptr_t, operatable_id_t> * data_arr, size_t sz) noexcept{
                    ...
                    auto src_logit_vmamap_reacquirer                = dg::network_exception_handler::nothrow_log(dg::network_vmamap::reacquirer_raii_initialize());
                    auto src_grad_vmamap_reacquirer                 = dg::network_exception_handler::nothrow_log(dg::network_vmamap::reacquirer_raii_initialize()); 
                    auto localcounterpart_grad_vmamap_reacquirer    = dg::network_exception_handler::nothrow_log(dg::network_vmamap::reacquirer_raii_initialize());
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        if (!src_bwd_operatable_id.has_value() || !src_init_status.has_value() || !src_grad_umaptr.has_value() || !src_logit_umaptr.has_value() || !src_grad_status.has_value()){
                            continue;
                        }

                        //^^^

                        //compatibility check required - we cant rely on users to rationally call orphan/deinit orders
                        //we can, however, rely on the system to have correct tile states at all time (such is a misoperation does not crash)

                        std::expected<uma_ptr_t, exception_t> prchk = dg::network_tile_member_access::safecthrow_extndst_ptr_access(localcounterpart);
                        ...
                        if (localcounterpart_operatable_id != expected_ops_id){
                            continue;
                        }
                        ...
                        if (!dg::network_uma::reacquirer_is_reacquirable(umamap_reacquirer, {{src_grad_umaptr.value(), dispatch_info.src_grad_vd_id},
                                                                                             {src_logit_umaptr.value(), dispatch_info.src_logit_vd_id},
                                                                                             {localcounterpart_grad_umaptr, dispatch_info.dst_grad_vd_id}})){
                        ...
                        vma_ptr_t src_logit_vmaptr              = dg::network_uma::get_vma_ptr(umamap_reacquirer, std::integral_constant<size_t, 0u>{});
                        vma_ptr_t src_grad_vmaptr               = dg::network_uma::get_vma_ptr(umamap_reacquirer, std::integral_constant<size_t, 1u>{});
                        vma_ptr_t localcounterpart_grad_vmaptr  = dg::network_uma::get_vma_ptr(umamap_reacquirer, std::integral_constant<size_t, 2u>{});

                        if (!dg::network_vmamap::reacquirer_is_region_reacquirable(src_grad_vmamap_reacquirer, src_grad_vmaptr)
                            || !dg::network_vmamap::reacquirer_is_region_reacquirable(src_logit_vmamap_reacquirer, src_logit_vmaptr)
                            || !dg::network_vmamap::reacquirer_is_region_reacquirable(localcounterpart_grad_vmamap_reacquirer, localcounterpart_grad_vmaptr)){
                        ...
                        dg::network_vmamap::reacquirer_reacquire_nothrow(src_grad_vmamap_reacquirer, src_grad_vmaptr);
                        dg::network_vmamap::reacquirer_reacquire_nothrow(src_logit_vmamap_reacquirer, src_logit_vmaptr);
                        dg::network_vmamap::reacquirer_reacquire_nothrow(localcounterpart_grad_vmamap_reacquirer, localcounterpart_grad_vmaptr);

                        if (dg::network_dispatch_control::is_cuda_dispatch(dispatch_info.dispatch_platform)){
                            cuda_ptr_t src_grad_cudaptr                 = dg::network_vmamap::get_cuda_ptr(src_grad_vmamap_reacquirer);
                            cuda_ptr_t src_logit_cudaptr                = dg::network_vmamap::get_cuda_ptr(src_logit_vmamap_reacquirer);
                            cuda_ptr_t localcounterpart_grad_cudaptr    = dg::network_vmamap::get_cuda_ptr(localcounterpart_grad_vmamap_reacquirer);

                            dg::network_producer_consumer::delvrsrv_deliver(cuda_delivery_handle.get(), src_logit_cudaptr, std::make_tuple(src_grad_cudaptr, src_logit_cudaptr, localcounterpart_grad_cudaptr, dispatch_info.tileops_cuda_dispatch_control, src_grad_status.value()));                
                        } else if (dg::network_dispatch_control::is_host_dispatch(dispatch_info.dispatch_platform)){
                            host_ptr_t src_grad_hostptr                 = dg::network_vmamap::get_host_ptr(src_grad_vmamap_reacquirer);
                            host_ptr_t src_logit_hostptr                = dg::network_vmamap::get_host_ptr(src_logit_vmamap_reacquirer);
                            host_ptr_t localcounterpart_grad_hostptr    = dg::network_vmamap::get_host_ptr(localcounterpart_grad_vmamap_reacquirer);

                            dg::network_producer_consumer::delvrsrv_deliver(host_delivery_handle.get(), src_logit_hostptr, std::make_tuple(src_grad_hostptr, src_logit_hostptr, localcounterpart_grad_hostptr, dispatch_info.tileops_host_dispatch_control, src_grad_status.value()));
                        } else{
                            if constexpr(DEBUG_MODE_FLAG){
        ...
        private:
            ...
            struct InternalResolutor: dg::network_producer_consumer::KVConsumerInterface<uma_ptr_t, std::tuple<uma_ptr_t, operatable_id_t>>{
                ...
                void push(uma_ptr_t lck_addr, std::tuple<uma_ptr_t, operatable_id_t> * data_arr, size_t sz) noexcept{
                    ...
                    auto dst_grad_vmamap_reacquirer     = dg::network_exception_handler::nothrow_log(dg::network_vmamap::reacquirer_raii_initialize());
                    auto host_synchronizer              = dg::network_exception_handler::nothrow_log(dg::network_exception::cstyle_initialize<dg::network_host_asynchronous::Synchronizer>());
                    auto extndst_tile_vec               = dg::vector<std::optional<ExtnDstTile>>(sz, std::optional<ExtnDstTile>(std::nullopt)); 

                    for (size_t i = 0u; i < sz; ++i){
                        auto [dst, expected_ops_id]         = data_arr[i];
                        init_status_t dst_init_status       = dg::network_tile_member_getsetter::get_extndst_init_status_nothrow(dst);
                        operatable_id_t dst_operatable_id   = dg::network_tile_member_getsetter::get_extndst_operatable_memevent_id_nothrow(dst);
                        ...
                        if (!dg::network_uma::reacquirer_fixedsize_is_region_reacquirable(umamap_reacquirer, {{dst_logit_umaptr, dispatch_info.dst_logit_vd_id}, {dst_grad_umaptr, dispatch_info.dst_grad_vd_id}})){
                            host_synchronizer.sync();
                        }

                        dg::network_uma::reacquirer_reacquire_nothrow(umamap_reacquirer, {{dst_logit_umaptr, dispatch_info.dst_logit_vd_id}, {dst_grad_umaptr, dispatch_info.dst_grad_vd_id}});
                        ...
                        if (dg::dg::network_dispatch_control::is_host_dispatch(dispatch_info.dispatch_platform)){
                            host_ptr_t dst_logit_hostptr    = dg::network_vmamap::get_host_ptr(dst_logit_vmamap_reacquirer);
                            host_ptr_t src_logit_hostptr    = dg::network_vmamap::get_host_ptr(dst_grad_vmamap_reacquirer);
                            
                            auto executable = []() noexcept{

                            };
                            auto async_task = dg::network_host_asynchronous::virtualize_async_task(std::move(executable));
                            auto async_id   = dg::network_exception_handler::nothrow_log(this->host_async_device->exec(std::move(async_task)));
                            dg::network_exception_handler::nothrow_log(synchronizer.add(async_id));
                        } else{
                            if constexpr(DEBUG_MODE_FLAG){
                                dg::network_log_stackdump::critical(dg::network_exception::verbose(dg::network_exception::INTERNAL_CORRUPTION));
                                std::abort();
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        if (!extndst_tile_vec[i].has_value()){
                            continue;
                        }

                        auto rq     = Request<external_virtual_memory_event_t>{};
                        rq.fr       = this->host_ip_retriever->ip();
                        std::expected<Address, exception_t> to_addr = this->uma_ip_retriever->ip(extndst_tile_vec[i]->counterpart); 
                        ...
                        rq.to       = to_addr.value();
                        rq.content  = dg::network_external_memcommit_factory::make_sequential_event(dg::network_external_memcommit_factory::make_event_shadow_injection(extndst_tile_vec[i]->counterpart, TILE_KIND_EXTNDST, dg::network_compact_serializer::serialize<dg::string>(extndst_tile_vec[i].value())), //bad cache thrashing
                                                                                                    dg::network_external_memcommit_factory::make_event_backward_do_signal(extndst_tile_vec[i]->counterpart, expected_ops_id));
        ...
        private:
                ...
                void push(uma_ptr_t lck_addr, std::tuple<uma_ptr_t, operatable_id_t, std::optional<uma_ptr_t> * > * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        switch (init_status){
                            ...
                            case TILE_INIT_STATUS_DECAYED:
                            {
                                *fetching_addr = std::nullopt; //optimizables
                                break;
                            }
                            case TILE_INIT_STATUS_INITIALIZED:
                            {
            ...
            struct InternalCudaResolutor: dg::network_producer_consumer::KVConsumerInterface<cuda_ptr_t, std::tuple<cuda_ptr_t, cuda_ptr_t, cuda_ptr_t, grad_status_t, cuda_tileops_dispatch_control_t>>{
                ...
                void push(cuda_ptr_t, std::tuple<cuda_ptr_t, cuda_ptr_t, cuda_ptr_t, grad_status_t, cuda_tileops_dispatch_control_t> * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        cuda_ptr_arr[i * 3 + 2] = dst_grad_cudaptr;
                        dg::network_exception_handler::nothrow_log(dg::network_tileops_cuda_poly::backward_mono_dispatch_chk(src_grad_cudaptr, src_logit_cudaptr, dst_grad_cudaptr, tileops_dispatch_control));
                        total_complexity        += dg::network_tileops_cuda_poly::decode_backward_mono_dispatch_control(tileops_dispatch_control)->runtime_complexity;
                        auto work_order         = [src_grad_cudaptr, src_logit_cudaptr, dst_grad_cudaptr, grad_status, tileops_dispatch_control]() noexcept{
                            dg::network_exception_handler::nothrow_log(dg::network_tileops_cuda_poly::backward_mono());
                        };
                        auto virtual_wo         = dg::network_cuda_controller::virtualize_async_task(std::move(work_order));
                        dg::network_exception_handler::nothrow_log(virtual_wo_vec->add(std::move(virtual_wo)));
            ...
            struct InternalHostResolutor: dg::network_producer_consumer::KVConsumerInterface<host_ptr_t, std::tuple<host_ptr_t, host_ptr_t, host_ptr_t, grad_status_t, host_tileops_dispatch_control_t>>{
                ...
                void push(host_ptr_t, std::tuple<host_ptr_t, host_ptr_t, host_ptr_t, grad_status_t, host_tileops_dispatch_control_t> * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        host_ptr_arr[i * 3 + 2] = dst_grad_hostptr;
                        dg::network_exception_handler::nothrow_log(dg::network_tileops_host_poly::backward_mono_dispatch_chk(src_grad_hostptr, src_logit_hostptr, dst_grad_hostptr, tileops_dispatch_control));
                        total_complexity        += dg::network_tileops_host_poly::decode_backward_mono_dispatch_control(tileops_dispatch_control)->runtime_complexity;
                        auto work_order         = [src_grad_hostptr, src_logit_hostptr, dst_grad_hostptr, grad_status, tileops_dispatch_control]() noexcept{
                            dg::network_exception_handler::nothrow_log(dg::network_tileops_host_poly::backward_mono());
                        };
                        auto virtual_wo         = dg::network_host_asynchronous::virtualize_async_task(std::move(work_order));
                        dg::network_exception_handler::nothrow_log(virtual_wo_vec->add(std::move(virtual_wo)));
            ...
            struct InternalResolutor: dg::network_producer_consumer::KVConsumerInterface<std::tuple<uma_ptr_t, uma_ptr_t>, std::tuple<uma_ptr_t, uma_ptr_t, operatable_id_t>>{
                ...
                void push(std::tuple<uma_ptr_t, uma_ptr_t> lck_addr, std::tuple<uma_ptr_t, uma_ptr_t, operatable_id_t> * data_arr, size_t sz) noexcept{
                    ...
                    auto umamap_reacquirer                          = dg::network_exception_handler::nothrow_log(dg::network_uma::reacquirer_fixedsize_raii_initialize(std::integral_constant<size_t, 3>{}));
                    auto src_grad_vmamap_reacquirer                 = dg::network_exception_handler::nothrow_log(dg::network_vmamap::reacquirer_raii_initialize());
                    auto src_logit_vmamap_reacquirer                = dg::network_exception_handler::nothrow_log(dg::network_vmamap::reacquirer_raii_initialize());
                    auto dst_grad_vmamap_reacquirer                 = dg::network_exception_handler::nothrow_log(dg::network_vmamap::reacquirer_raii_initialize());
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        if (!src_init_status.has_value() || !src_bwd_operatable_id.has_value() || !src_grad_umaptr.has_value() || !src_logit_umaptr.has_value()){
                            continue;
                        }
                        ...
                        if (!dg::network_umamap::reacquirer_is_reacquirable(umamap_reacquirer, {{src_grad_umaptr.value(), dispatch_info.src_grad_vd_id},
                                                                                                {src_logit_umaptr.value(), dispatch_info.src_logit_vd_id},
                                                                                                {dst_grad_umaptr, dispatch_info.dst_grad_vd_id}})){
                        ...
                        dg::network_umamap::reacquirer_reacquire_nothrow(umamap_reacquirer, {{src_grad_umaptr.value(), dispatch_info.src_grad_vd_id},
                                                                                             {src_logit_umaptr.value(), dispatch_info.src_logit_vd_id},
                                                                                             {dst_grad_umaptr, dispatch_info.dst_grad_vd_id}});
                        ...
                        if (dg::dg::network_dispatch_control::is_cuda_dispatch(dispatch_info.dispatch_platform)){
                            ...
                            dg::network_producer_consumer::delvrsrv_deliver(host_delivery_handle.get(), src_logit_hostptr, std::make_tuple());
                        } else{
                            if constexpr(DEBUG_MODE_FLAG){
                                dg::network_log_stackdump::critical(dg::network_exception::verbose(dg::network_exception::INTERNAL_CORRUPTION));
                                std::abort();
    ...
    class BackwardDoMsgrFwdSignalResolutorV2: public virtual dg::network_producer_consumer::ConsumerInterface<BackwardDoSignalEvent>{
        ...
        private:
            ...
            struct InternalDescendantAddressFetcher: dg::network_producer_consumer::KVConsumerInterface<uma_ptr_t, std::tuple<uma_ptr_t, operatable_id_t, std::optional<uma_ptr_t> *>>{
                ...
                void push(uma_ptr_t rcu_addr, std::tuple<uma_ptr_t, operatable_id_t, std::optional<uma_ptr_t> *> * ptr_arr, size_t sz) noexcept{
            ...
            struct InternalCudaResolutor: dg::network_producer_consumer::KVConsumerInterface<cuda_ptr_t, std::tuple<cuda_ptr_t, cuda_ptr_t, cuda_ptr_t, grad_status_t, cuda_tileops_dispatch_control_t>>{
                ...
                void push(cuda_ptr_t, std::tuple<cuda_ptr_t, cuda_ptr_t, cuda_ptr_t, grad_status_t, cuda_tileops_dispatch_control_t> * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        dg::network_exception_handler::nothrow_log(dg::network_tileops_cuda_poly::backward_mono_dispatch_chk(src_grad_cudaptr, src_logit_cudaptr, dst_grad_cudaptr, tileops_dispatch_control));
                        total_complexity        += dg::network_tileops_cuda_poly::decode_backward_mono_dispatch_control(tileops_dispatch_control)->runtime_complexity;
                        auto work_order         = [src_grad_cudaptr, src_logit_cudaptr, dst_grad_cudaptr, src_grad_status, tileops_dispatch_control]() noexcept{
                            dg::network_exception_handler::nothrow_log(dg::network_tileops_cuda_poly::backward_mono());
                        };
                        auto virtual_wo         = dg::network_cuda_controller::virtualize_async_task(std::move(work_order));
                        dg::network_exception_handler::nothrow_log(virtual_wo_vec->add(std::move(virtual_wo)));
            ...
            struct InternalHostResolutor: dg::network_producer_consumer::KVConsumerInterface<host_ptr_t, std::tuple<host_ptr_t, host_ptr_t, host_ptr_t, grad_status_t, host_tileops_dispatch_control_t>>{
                ...
                void push(host_ptr_t, std::tuple<host_ptr_t, host_ptr_t, host_ptr_t, grad_status_t, host_tileops_dispatch_control_t> * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        dg::network_exception_handler::nothrow_log(dg::network_tileops_host_poly::backward_mono_dispatch_chk(src_grad_hostptr, src_logit_hostptr, dst_grad_hostptr, tileops_dispatch_control));
                        total_complexity        += dg::network_tileops_host_poly::decode_backward_mono_dispatch_control(tileops_dispatch_control)->runtime_complexity;
                        auto work_order         = [src_grad_hostptr, src_logit_hostptr, dst_grad_hostptr, src_grad_status, tileops_dispatch_control]() noexcept{
                            dg::network_exception_handler::nothrow_log(dg::network_tileops_host_poly::backward_mono());
                        };
                        auto virtual_wo         = dg::network_host_asynchronous::virtualize_async_task(std::move(work_order));
                        dg::network_exception_handler::nothrow_log(virtual_wo_vec->add(std::move(virtual_wo)));
            ...
            struct InternalResolutor: dg::network_producer_consumer::KVConsumerInterface<std::tuple<uma_ptr_t, uma_ptr_t>, std::tuple<uma_ptr_t, uma_ptr_t, operatable_id_t>>{
                ...
                void push(std::tuple<uma_ptr_t, uma_ptr_t> lck_addr, std::tuple<uma_ptr_t, uma_ptr_t, operatable_id_t> * data_arr, size_t sz) noexcept{
                    ...
                    size_t cdh_allocation_cost                      = dg::network_producer_consumer::delvrsrv_kv_allocation_cost(&cuda_internal_resolutor, trimmed_cuda_vectorization_sz);
                    dg::network_stack_allocation::NoExceptRawAllocation<char[]> cdh_mem(cdh_allocation_cost);
                    auto cuda_delivery_handle                       = dg::network_exception_handler::nothrow_log(dg::network_producer_consumer::delvsrv_kv_open_preallocated_raiihandle(&cuda_internal_resolutor, trimmed_cuda_vectorization_sz));

                    size_t trimmed_host_vectorization_sz            = std::min(this->vectorization_sz, sz);
                    size_t hdh_allocation_cost                      = dg::network_producer_consumer::delvrsrv_kv_allocation_cost(&host_internal_resolutor, trimmed_host_vectorization_sz);
                    dg::network_stack_allocation::NoExceptRawAllocation<char[]> hdh_mem(hdh_allocation_cost);
                    auto host_delivery_handle                       = dg::network_exception_handler::nothrow_log(dg::network_producer_consumer::delvsrv_open_kv_preallocated_raiihandle(&host_internal_resolutor, trimmed_host_vectorization_sz)); 
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        if (dg::dg::network_dispatch_control::is_cuda_dispatch(dispatch_info.dispatch_platform)){
                            ...
                            dg::network_producer_consumer::delvrsrv_deliver(cuda_delivery_handle.get(), src_logit_cudaptr, std::make_tuple(src_grad_cudaptr, src_logit_cudaptr, dst_grad_cudaptr, dispatch_info.tileops_cuda_dispatch_control));
                        } else if (dg::dg::network_dispatch_control::is_host_dispatch(dispatch_info.dispatch_platform)){
                            host_ptr_t src_grad_hostptr     = dg::network_vmamap::get_host_ptr(src_grad_vmamap_reacquirer);
                            host_ptr_t src_logit_hostptr    = dg::network_vmamap::get_host_ptr(src_logit_vmamap_reacquirer);
                            host_ptr_t dst_grad_hostptr     = dg::network_vmamap::get_host_ptr(dst_grad_vmamap_reacquirer);

                            dg::network_producer_consumer::delvrsrv_deliver(host_delivery_handle.get(), src_logit_hostptr, std::make_tuple(src_grad_hostptr, src_logit_hostptr, dst_grad_hostptr, dispatch_info.tileops_host_dispatch_control));
                        } else{
                            if constexpr(DEBUG_MODE_FLAG){
                                dg::network_log_stackdump::critical(dg::network_exception::verbose(dg::network_exception::INTERNAL_CORRUPTION));
                                std::abort();
            ...
            struct InternalCudaResolutor: dg::network_producer_consumer::KVConsumerInterface<cuda_ptr_t, std::tuple<cuda_ptr_t, cuda_ptr_t, cuda_ptr_t, grad_status_t, cuda_tileops_dispatch_control_t>>{
                ...
                void push(cuda_ptr_t, std::tuple<cuda_ptr_t, cuda_ptr_t, cuda_ptr_t, grad_status_t, cuda_tileops_dispatch_control_t> * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        dg::network_exception_handler::nothrow_log(dg::network_tileops_cuda_poly::backward_mono_dispatch_chk(src_grad_cudaptr, src_logit_cudaptr, dst_grad_cudaptr, tileops_dispatch_control));
                        total_complexity        += dg::network_tileops_cuda_poly::decode_backward_mono_dispatch_control(tileops_dispatch_control)->runtime_complexity;
                        auto work_order         = [src_grad_cudaptr, src_logit_cudaptr, dst_grad_cudaptr, src_grad_status, tileops_dispatch_control](){
                            dg::network_exception_handler::nothrow_log(dg::network_tileops_cuda_poly::backward_mono());
                        };
                        auto virtual_wo         = dg::network_cuda_controller::virtualize_async_task(std::move(work_order));
                        dg::network_exception_handler::nothrow_log(virtual_wo_vec->add(std::move(virtual_wo)));
            ...
            struct InternalResolutor: dg::network_producer_consumer::KVConsumerInterface<std::tuple<uma_ptr_t, uma_ptr_t>, std::tuple<uma_ptr_t, uma_ptr_t>>{
                ...
                void push(std::tuple<uma_ptr_t, uma_ptr_t> lck_addr, std::tuple<uma_ptr_t, uma_ptr_t, operatable_id_t> * data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        if (!dg::network_uma::reacquirer_fixedsize_is_region_reacquirable(umamap_reacquirer, {{src_grad_umaptr.value(), dispatch_info.src_grad_vd_id},
                                                                                                              {src_logit_umaptr.value(), dispatch_info.src_logit_vd_id}, 
                                                                                                              {dst_grad_umaptr, dispatch_info.dst_grad_vd_id}})){
                            dg::network_producer_consumer::delvrsrv_clear(host_vectorizer.get());
                            host_synchronizer.sync();
                        ...
                        dg::network_uma::reacquirer_fixedsize_reacquire_nothrow(umamap_reacquirer, {{src_grad_umaptr.value(), dispatch_info.src_grad_vd_id}, 
                                                                                                    {src_logit_umaptr.value(), dispatch_info.src_logit_vd_id}, 
                                                                                                    {dst_grad_umaptr, dispatch_info.dst_grad_vd_id}});
                        ...
                        if (dg::network_dispatch_control::is_cuda_dispatch(dispatch_info.dispatch_platform)){
                            ...
                            dg::network_producer_consumer::delvrsrv_deliver(cuda_vectorizer.get(), src_logit_cudaptr, std::make_tuple(src_grad_cudaptr, src_logit_cudaptr, dst_grad_cudaptr, src_grad_status, dispatch_info.tileops_cuda_dispatch_control));
                        } else if (dg::network_dispatch_control::is_host_dispatch(dispatch_info.dispatch_platform)){
                            host_ptr_t src_grad_hostptr     = dg::network_vmamap::get_host_ptr(src_grad_vmamap_reacquirer);
                            host_ptr_t src_logit_hostptr    = dg::network_vmamap::get_host_ptr(src_logit_vmamap_reacquirer);
                            host_ptr_t dst_grad_hostptr     = dg::network_vmamap::get_host_ptr(dst_grad_vmamap_reacquirer);

                            dg::network_producer_consumer::delvrsrv_deliver(host_vectorizer.get(), src_logit_hostptr, std::make_tuple(src_grad_hostptr, src_logit_hostptr, dst_grad_hostptr, src_grad_status, dispatch_info.tileops_host_dispatch_control));
                        } else{
                            if constexpr(DEBUG_MODE_FLAG){
                                dg::network_log_stackdump::critical(dg::network_exception::verbose(dg::network_exception::INTERNAL_CORRUPTION));
                                std::abort();
