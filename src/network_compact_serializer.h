...
namespace dg::network_compact_serializer{
    ...
    template <class T>
    auto deserialize_into(T& obj, const char * buf) -> const char *{ //not sfinae-qualified 
    ...
    template <class T>
    auto integrity_size(const T& obj) noexcept -> size_t{ //not sfinae-qualified

        return static_cast<size_t>(sizeof(types::hash_type)) + count(obj);
    }

    template <class T>
    auto integrity_serialize_into(char * buf, const T& obj) noexcept -> char *{ //not sfinae-qualified 
    ...
    template <class T>
    auto integrity_deserialize_into(T& obj, const char * buf, size_t sz) -> std::expected<const char *, exception_t> { //not sfinae-qualified 
