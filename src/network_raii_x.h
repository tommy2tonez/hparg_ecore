...
namespace dg{
    ...
    class inline_unique_resource{
        ...
        public:
            ...
            using self = inline_unique_resource;

            template <class DelArg = ResourceDeallocator, std::enable_if_t<std::is_nothrow_default_constructible_v<DelArg>, bool> = true>
            inline_unique_resource() noexcept: resource(), 
                                               deallocator(), 
                                               responsibility_flag(false){}

            inline_unique_resource(ResourceType resource, ResourceDeallocator deallocator) noexcept: resource(resource),
                                                                                                     deallocator(std::move(deallocator)),
                                                                                                     responsibility_flag(true){}
            
            inline_unique_resource(const self&) = delete;
            inline_unique_resource(self&& other) noexcept: resource(other.resource),
                                                           deallocator(std::move(other.deallocator)),
                                                           responsibility_flag(other.responsibility_flag){
                other.responsibility_flag = false;
            }

            __attribute__((always_inline)) ~inline_unique_resource() noexcept{
